...
def generateFutures(): Unit = {

  genJavaslangFile("javaslang.control", "Futures"){(im: ImportManager, packageName: String, className: String) =>

    val Future = im.getType("javaslang.control.Future")
    val List = im.getType("javaslang.collection.List")
    val Try = im.getType("javaslang.control.Try")
    val Promise = im.getType("javaslang.control.Promise")
    val Executor = im.getType("java.util.concurrent.Executor")
    val ForkJoinPool = im.getType("java.util.concurrent.ForkJoinPool")
    val Function1 = im.getType("javaslang.Function1")
    val Objects = im.getType("java.util.Objects")

    im.getType("javaslang.Tuple")
    im.getType("javaslang.Tuple2")
    im.getType("javaslang.Tuple3")


    xs"""
      |/**
      | * A helper class for forming Iterables and Tuples of Futures into Futures of Iterables or Tuples
      | *
      | *
      | *
      | */
