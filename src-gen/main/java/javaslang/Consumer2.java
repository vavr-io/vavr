 ...
 */
@FunctionalInterface
public interface Consumer2<T1, T2> extends λ<Void> {
    ...
    @Override
    default Function1<T1, Consumer1<T2>> curried() {
        return t1 -> t2 -> accept(t1, t2);
    }

    @Override
    default Consumer1<Tuple2<T1, T2>> tupled() {
    ...
    /**
     * Returns a composed function that first applies this Consumer2 to the given arguments and then applies
     ...
     * this Consumer2 to the same arguments.
     *
     ...
     */
    default Consumer2<T1, T2> compose(Consumer2<T1, T2> before) {
