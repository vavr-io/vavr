...
/**
 * <p>Defines a 9-ary Functor by generalizing the map function which maps 9 elements.</p>
 *
 * All instances of the Functor9 interface should obey the two functor laws:
 * <ul>
 *     <li>{@code m.map(a -> a) ≡ m}</li>
 *     <li>{@code m.map(f.compose(g)) ≡ m.map(g).map(f)}</li>
 * </ul>
 * where {@code f, g ∈ Tuple9 → Tuple9}.
 *
...
public interface Functor9<T1, T2, T3, T4, T5, T6, T7, T8, T9> {
     ...
     * @param f a 9-ary Function which maps the components of this Functor
     * @return a new Functor9 with 9 component types U1, U2, U3, U4, U5, U6, U7, U8, U9.
     */
