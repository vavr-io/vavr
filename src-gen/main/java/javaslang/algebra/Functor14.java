...
/**
 * <p>Defines a 14-ary Functor by generalizing the map function which maps 14 elements.</p>
 *
 * All instances of the Functor14 interface should obey the two functor laws:
 * <ul>
 *     <li>{@code m.map(a -> a) ≡ m}</li>
 *     <li>{@code m.map(f.compose(g)) ≡ m.map(g).map(f)}</li>
 * </ul>
 * where {@code f, g ∈ Tuple14 → Tuple14}.
 *
...
public interface Functor14<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12, T13, T14> {
     ...
     * @param f a 14-ary Function which maps the components of this Functor
     * @return a new Functor14 with 14 component types U1, U2, U3, U4, U5, U6, U7, U8, U9, U10, U11, U12, U13, U14.
     */
