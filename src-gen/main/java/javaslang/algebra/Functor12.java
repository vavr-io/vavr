...
/**
 * <p>Defines a 12-ary Functor by generalizing the map function which maps 12 elements.</p>
 *
 * All instances of the Functor12 interface should obey the two functor laws:
 * <ul>
 *     <li>{@code m.map(a -> a) ≡ m}</li>
 *     <li>{@code m.map(f.compose(g)) ≡ m.map(g).map(f)}</li>
 * </ul>
 * where {@code f, g ∈ Tuple12 → Tuple12}.
 *
...
public interface Functor12<T1, T2, T3, T4, T5, T6, T7, T8, T9, T10, T11, T12> {
     ...
     * @param f a 12-ary Function which maps the components of this Functor
     * @return a new Functor12 with 12 component types U1, U2, U3, U4, U5, U6, U7, U8, U9, U10, U11, U12.
     */
