...
public class FutureTest extends AbstractValueTest {
  ...
  static class TraceUnitExtension implements AfterEachCallback, BeforeEachCallback {
    ...
    private void printForkJoinPoolInfo() {
      ...
      final String info =
          String.format(
              "- [ForkJoinPool.commonPool()] parallelism: %s, poolSize: %s, isAsyncMode: %s, runningThreadCount: %s, activeThreadCount: %s, isQuiescent: %s, stealCount: %s, queuedTaskCount: %s, queuedSubmissionCount: %s, hasQueuedSubmissions: %s",
              pool.getParallelism(),
              pool.getPoolSize(),
              pool.getAsyncMode(),
    ...
    return new IterableAssert<T>(actual) {
      ...
      public IterableAssert<T> isEqualTo(Object expected) {
        if (actual instanceof Future && expected instanceof Future) {
          FutureTest.super
              .assertThat(((Future<T>) actual).getValue())
  ...
  public void shouldCreateFutureFromJavaFutureUsingTrivialExecutor() {
    final java.util.concurrent.Future<String> jFuture =
  ...
  public void shouldCreateFutureFromLateFailingJavaCompletableFuture() {
    final CompletableFuture<Integer> jFuture =
        Future.<Integer>of(zZz(new RuntimeException())).toCompletableFuture();
    final Future<Integer> future = Future.fromCompletableFuture(jFuture).await();
    assertFailed(future, RuntimeException.class);
  }
  ...
  @Test
  public void
      shouldFailFindingFirstValueBecauseNoResultSatisfiesTheGivenPredicateUsingForkJoinPool() {
    final Seq<Future<Integer>> futures = Stream.from(1).map(i -> Future.of(() -> i)).take(20);
  ...
  public void shouldFindOneSucceedingFutureWhenAllOthersFailUsingDefaultExecutor() {
    final Seq<Future<Integer>> futures =
        Stream.from(1)
            .map(
                i ->
                    Future.<Integer>of(
  ...
  public void shouldGetFirstCompletedOfFailuresUsingForkJoinPool() {
    final Seq<Future<Object>> futures =
        Stream.from(1).map(i -> Future.of(zZz(new Error()))).take(3);
  ...
  public void shouldCompleteOneFutureUsingAThreadPoolExecutorLimitedToOneThread() {
    final ExecutorService service =
        new ThreadPoolExecutor(1, 1, 0L, MILLISECONDS, new SynchronousQueue<>());
  ...
  public void shouldCompleteThreeFuturesUsingAThreadPoolExecutorLimitedToTwoThreads() {
    final ExecutorService service =
        new ThreadPoolExecutor(1, 2, 0L, MILLISECONDS, new SynchronousQueue<>());
    final Stream<Future<Integer>> futures =
        Stream.rangeClosed(1, 3).map(value -> Future.of(service, () -> expensiveOperation(value)));
  ...
  public void shouldFailReduceEmptySequence() {
    assertThrows(
        NoSuchElementException.class, () -> Future.<Integer>reduce(List.empty(), Integer::sum));
  }

  @Test
  public void shouldReduceSequenceOfFutures() {
    final Future<String> future =
        Future.reduce(List.of(Future.of(zZz("Va")), Future.of(zZz("vr"))), (i1, i2) -> i1 + i2)
            .await();
  ...
  public void shouldReduceWithErrorIfSequenceOfFuturesContainsOneError() {
    final Future<Integer> future =
        Future.reduce(List.of(Future.of(zZz(13)), Future.of(zZz(new Error()))), Integer::sum)
            .await();
    assertFailed(future, Error.class);
  }
  ...
  public void shouldCompleteWithSeqOfValueIfSequenceOfFuturesContainsNoError() {
    final Future<Seq<Integer>> sequence =
        Future.sequence(List.of(Future.of(zZz(1)), Future.of(zZz(2)))).await();
  ...
  public void shouldCompleteWithErrorIfSequenceOfFuturesContainsOneError() {
    final Future<Seq<Integer>> sequence =
        Future.sequence(List.of(Future.of(zZz(13)), Future.of(zZz(new Error())))).await();
  ...
  public void shouldCompleteTraverse() {
    final Future<Seq<Integer>> future =
        Future.traverse(List.of(1, 2, 3), i -> Future.of(zZz(i))).await();
  ...
  public void shouldCompleteWithErrorIfFailAndThenFail() {
    final Future<Integer> future =
        Future.<Integer>of(zZz(new Error("fail!")))
  ...
  public void shouldCompleteWithSuccessIfSuccessAndThenFail() {
    final Future<Integer> future =
        Future.of(zZz(42)).andThen(t -> zZz(new Error("and then fail!"))).await();
  ...
  public void shouldCompleteWithSpecificOrderIfSuccessAndThenSuccess() {
    ...
    final Future<Void> future =
        Future.run(() -> Thread.sleep(250)).andThen(t -> sideEffect[0] = 42);
  ...
  public void shouldHandleInterruptedExceptionCorrectlyInAwait() {
    // the Future should never be completed as long as the InterruptedException is rethrown by the
    // Try...
    final Future<Void> future =
        Future.run(
            () -> {
  ...
  public void shouldConvertToFailedFromSuccess() {
    ...
    assertThat(future.isFailure()).isTrue();
    assertThat(future.getValue().get().getCause().getClass())
  ...
  public void shouldFoldNonEmptyIterableOfFailingFutures() {
    final Seq<Future<Integer>> futures =
        Stream.from(1).map(i -> Future.<Integer>of(zZz(new Error()))).take(5);
    final Future<Integer> testee = Future.fold(futures, 0, Integer::sum).await();
  ...
  public void shouldCollectDefinedValueUsingPartialFunction() {
    final PartialFunction<Integer, String> pf =
        Function1.<Integer, String>of(String::valueOf).partial(i -> i % 2 == 1);
  ...
  public void shouldNotCrashWhenRecoverFails() {
    final Future<Integer> recovered =
        Future.<Integer>of(zZz(new Error()))
            .recover(
                t -> {
  ...
  public void shouldRecoverSuccessFutureWithFuture() {
    final Future<String> recovered =
        Future.of(() -> "oh!").recoverWith(ignored -> Future.of(() -> "ignored"));
    waitUntil(recovered::isCompleted);
    assertThat(recovered.isSuccess()).isTrue();
    assertThat(recovered.get()).isEqualTo("oh!");
  }
  ...
  public void shouldConvertFailedFutureToCompletableFuture() {
    final CompletableFuture<Integer> completableFuture =
        Future.<Integer>failed(new RuntimeException()).toCompletableFuture();
    assertThat(completableFuture.isCompletedExceptionally());
    assertThat(Try.of(completableFuture::get).getCause())
  ...
  public void shouldTransformResultFromFailureToSuccess() {
    final Future<String> future =
        Future.of(zZz(new Error())).transformValue(t -> Try.of(() -> "forty two"));
    future.await();
    waitUntil(future::isSuccess);
    assertThat(future.get()).isEqualTo("forty two");
  }
  ...
  public void shouldZipFailure() {
    final Future<Tuple2<Integer, Integer>> future =
        Future.<Integer>of(zZz(new Error())).zip(Future.of(zZz(2)));
    future.await();
    waitUntil(future::isFailure);
    assertThat(future.getCause().get().getClass()).isEqualTo(Error.class);
  }
  ...
  public void shouldZipWithFailure() {
    final Future<List<Integer>> future =
        Future.<Integer>of(zZz(new Error())).zipWith(Future.of(zZz(2)), List::of);
    future.await();
    waitUntil(future::isFailure);
    assertThat(future.getCause().get().getClass()).isEqualTo(Error.class);
  }
