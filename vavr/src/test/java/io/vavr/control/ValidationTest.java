...
public class ValidationTest extends AbstractValueTest {
  ...
  public class CondTests {
    ...
    public void shouldNotEvaluateValidSupplierOnFalse() {
      Validation<String, Integer> validation =
          Validation.cond(
              false,
              () -> {
    ...
    public void shouldBeFineWithCovariantError() {
      Validation<Car, Integer> validation =
          Validation.cond(false, () -> 21, () -> new Hatchback("vavr"));
      assertThat(validation).isEqualTo(Validation.invalid(new Hatchback("vavr")));
    }

    @Test
    public void shouldBeFineWithCovariantValid() {
      Validation<String, Car> validation =
          Validation.cond(true, () -> new Sedan("vavr"), () -> "vavr");
  ...
  public void shouldCreateInvalidWhenSequencingAnInvalid() {
    ...
    assertThat(actual)
        .isEqualTo(Validation.invalid(List.of("error1", "error2", "error3", "error4")));
  }
  ...
  public void shouldCreateValidWhenTraversingValids() {
    final Validation<Seq<String>, Seq<Integer>> actual =
        Validation.traverse(
            ...
            t ->
                Validation.valid(
                    t)); // NOTE: Compilation error with Java 8 if we use a method reference
    assertThat(actual).isEqualTo(Validation.valid(List.of(1, 2)));
  }
  ...
  public void shouldCreateInvalidWhenTraversingAnInvalid() {
    ...
    assertThat(actual)
        .isEqualTo(Validation.invalid(List.of("error-1", "error0", "error-2", "error-1")));
  }
  ...
  public void shouldMapFailureErrorOnGet() {
    assertThrows(
        RuntimeException.class, () -> assertThat(invalid().map(s -> 2).get()).isEqualTo(ERRORS));
  }
  ...
  public void shouldMapOnlyFailureValue() {
    Validation<Seq<String>, String> invalidValidation = invalid();
    Validation<Integer, Integer> invalidMapping =
        invalidValidation.bimap(Seq::length, String::length);
  ...
  public void shouldBuildUpForSuccessCombine() {
    ...
    Validation<Seq<String>, TestValidation> result2 =
        v1.combine(v2).combine(v3).ap(TestValidation::new);
    Validation<Seq<String>, TestValidation> result3 =
        v1.combine(v2).combine(v4).ap(TestValidation::new);

    Validation<Seq<String>, TestValidation> result4 =
        v1.combine(v2).combine(v3).combine(v5).ap(TestValidation::new);
    Validation<Seq<String>, TestValidation> result5 =
        v1.combine(v2).combine(v3).combine(v5).combine(v6).ap(TestValidation::new);
    Validation<Seq<String>, TestValidation> result6 =
        v1.combine(v2).combine(v3).combine(v5).combine(v6).combine(v7).ap(TestValidation::new);
    Validation<Seq<String>, TestValidation> result7 =
        v1.combine(v2)
            .combine(v3)
            .combine(v5)
            .combine(v6)
            .combine(v7)
            .combine(v8)
            .ap(TestValidation::new);
    Validation<Seq<String>, TestValidation> result8 =
        v1.combine(v2)
            .combine(v3)
            .combine(v5)
            .combine(v6)
    ...
    Validation<Seq<String>, String> result9 =
        v1.combine(v2).combine(v3).ap((p1, p2, p3) -> p1 + ":" + p2 + ":" + p3.getOrElse("none"));
  ...
  public void shouldBuildUpForSuccessMapN() {
    ...
    Validation<Seq<String>, TestValidation> result =
        Validation.combine(v1, v2).ap(TestValidation::new);
    Validation<Seq<String>, TestValidation> result2 =
        Validation.combine(v1, v2, v3).ap(TestValidation::new);
    Validation<Seq<String>, TestValidation> result3 =
        Validation.combine(v1, v2, v4).ap(TestValidation::new);
    Validation<Seq<String>, TestValidation> result4 =
        Validation.combine(v1, v2, v3, v5).ap(TestValidation::new);
    Validation<Seq<String>, TestValidation> result5 =
        Validation.combine(v1, v2, v3, v5, v6).ap(TestValidation::new);
    Validation<Seq<String>, TestValidation> result6 =
        Validation.combine(v1, v2, v3, v5, v6, v7).ap(TestValidation::new);
    Validation<Seq<String>, TestValidation> result7 =
        Validation.combine(v1, v2, v3, v5, v6, v7, v8).ap(TestValidation::new);
    Validation<Seq<String>, TestValidation> result8 =
        Validation.combine(v1, v2, v3, v5, v6, v7, v8, v9).ap(TestValidation::new);

    Validation<Seq<String>, String> result9 =
        Validation.combine(v1, v2, v3)
            .ap((p1, p2, p3) -> p1 + ":" + p2 + ":" + p3.getOrElse("none"));
  ...
  public void shouldBuildUpForFailure() {
    ...
    Validation<Seq<String>, TestValidation> result =
        v1.combine(e2).combine(v3).ap(TestValidation::new);
    Validation<Seq<String>, TestValidation> result2 =
        e1.combine(v2).combine(e3).ap(TestValidation::new);
  ...
  static class TestValidation {
    ...
    TestValidation(
        String name, Integer age, Option<String> address, String phone, String alt1, String alt2) {
      this.name = name;
      this.age = age;
      this.address = address;
      this.phone = phone;
  ...
  public void shouldValidateInvalidPerson() {
    ...
    final int age = -1;
    final Validation<Seq<String>, Person> actual = new PersonValidator().validatePerson(name, age);
    final Validation<Seq<String>, Person> expected =
        Validation.invalid(
            List.of("Name contains invalid characters: '!4?'", "Age must be greater than 0"));
    assertThat(actual).isEqualTo(expected);
  }
  ...
  static class PersonValidator {
    ...
    private Validation<String, Integer> validateAge(int age) {
      return (age < minAge)
          ? Validation.invalid("Age must be greater than 0")
