...
public class OptionTest extends AbstractValueTest {
  ...
  public void shouldConvertListOfNonEmptyOptionsToOptionOfList() {
    final List<Option<String>> options =
        Arrays.asList(Option.of("a"), Option.of("b"), Option.of("c"));
  ...
  public void shouldConvertListOfMixedOptionsToOptionOfList() {
    final List<Option<String>> options =
        Arrays.asList(Option.of("a"), Option.none(), Option.of("c"));
    final Option<Seq<String>> option = Option.sequence(options);
    assertThat(option instanceof Option.None).isTrue();
  }
  ...
  class MapTry {
    ...
    public void shouldMapTryCheckedException() {
      Try<Integer> result = Option.of("a").mapTry(this::checkedFunction);
      assertThat(result.isFailure()).isTrue();
  ...
  public void shouldFlatMapNone() {
    assertThat(Option.<Integer>none().flatMap(i -> Option.of(String.valueOf(i))))
        .isEqualTo(Option.none());
  }
  ...
  public void shouldHandleTransformOnNone() {
    assertThat(Option.none().<String>transform(self -> self.isEmpty() ? "ok" : "failed"))
        .isEqualTo("ok");
  }
  ...
  public void shouldNotCallPartialFunctionOnUndefinedArg() {
    final PartialFunction<Integer, Integer> pf =
        Function1.<Integer, Integer>of(x -> 1 / x).partial(i -> i != 0);
