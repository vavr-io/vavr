...
public class EitherLeftProjectionTest extends AbstractValueTest {
  ...
  public void shouldLeftProjectionOrElseLeftProjection() {
    final Either.LeftProjection<Integer, Integer> elseProjection =
        API.<Integer, Integer>Left(2).left();
  ...
  public void shouldReturnLeftWhenOrElseThrowWithFunctionOnLeftProjectionOfLeft() {
    final Integer actual =
        Either.<Integer, String>left(1).left().getOrElseThrow(s -> new RuntimeException(s));
    assertThat(actual).isEqualTo(1);
  }
  ...
  public void shouldConvertLeftProjectionOfRightToJavaOptional() {
    assertThat(Either.<Integer, String>right("x").left().toJavaOptional())
  ...
  public void shouldFlatMapLeftProjectionOfRightOnLeftProjectionOfLeft() {
    ...
    final Either.LeftProjection<Tuple2<String, String>, String> actual =
        good.left().flatMap(g -> bad.left().map(b -> Tuple.of(g, b)));
  ...
  public void shouldPeekOnLeftProjectionOfRight() {
    final List<Integer> actual = new ArrayList<>();
    final Either<Integer, String> testee =
        Either.<Integer, String>right("1").left().peek(actual::add).toEither();
    assertThat(actual.isEmpty()).isTrue();
  ...
  public void shouldMapOnLeftProjectionOfLeft() {
    final Either<Integer, String> actual =
        Either.<Integer, String>left(1).left().map(i -> i + 1).toEither();
    assertThat(actual).isEqualTo(Either.left(2));
  }
  ...
  public void shouldMapOnLeftProjectionOfRight() {
    final Either<Integer, String> actual =
        Either.<Integer, String>right("1").left().map(i -> i + 1).toEither();
    assertThat(actual).isEqualTo(Either.right("1"));
  }
