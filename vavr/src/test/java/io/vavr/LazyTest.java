...
public class LazyTest extends AbstractValueTest {
  ...
  public void shouldBeConsistentFromMultipleThreads() throws Exception {
    for (int i = 0; i < 100; i++) {
      ...
      final Vector<CompletableFuture<Void>> futures =
          Vector.range(0, 10)
              .map(
                  j -> {
      ...
      final CompletableFuture all =
          CompletableFuture.allOf(futures.toJavaList().toArray(new CompletableFuture<?>[0]));
      canProceed.set(true);
      all.join();
    }
  }
 ...
 * In order to reuse existing unit tests, that are valid for all Value implementations, we provide
 * here an _imaginary_ empty Lazy implementation, called 'undefined'.
 *
 * <p>Note: It is no good idea to leak it outside of the test scope into the core library (otherwise
 * Undefined will be the new null).
 */
