...
public class MatchTest {
  ...
  public void shouldThrowIfNotMatching() {
    assertThrows(
        MatchError.class,
        () -> {
          Match(new Object()).of(Case($(ignored -> false), o -> null));
        });
  }
  ...
  public void shouldTakeFirstMatch() {
    final String actual = Match(new Object()).of(Case($(), "first"), Case($(), "second"));
  ...
  public void shouldMatchIntUsingPatterns() {
    final String actual = Match(3).of(Case($(1), "one"), Case($(2), "two"), Case($(), "many"));
  ...
  public void shouldMatchIntUsingPredicates() {
    final String actual =
        Match(3).of(Case($(is(1)), "one"), Case($(is(2)), "two"), Case($(), "many"));
    assertThat(actual).isEqualTo("many");
  }
  ...
  public void shouldComputeUpperBoundOfReturnValue() {
    final Number num =
        Match(3)
            .<Number>of(
                Case($(is(1)), 1), Case($(is(2)), 2.0), Case($(), i -> new BigDecimal("" + i)));
  ...
  public void shouldMatchLeft() {
    ...
    final String actual =
        Match(either)
            .of(Case($Left($()), l -> "left: " + l), Case($Right($()), r -> "right: " + r));
  ...
  public void shouldMatchSome() {
    ...
    final String actual =
        Match(opt).of(Case($None(), "no value"), Case($Some($()), String::valueOf));
    assertThat(actual).isEqualTo("1");
  }
  ...
  public void shouldMatchNone() {
    ...
    final String actual =
        Match(opt).of(Case($Some($()), String::valueOf), Case($None(), "no value"));
  ...
  public void shouldDecomposeSomeTuple() {
    ...
    final Tuple2<String, Integer> actual =
        Match(tuple2Option)
            .of(
                Case(
                    $Some($()),
  ...
  public void shouldDecomposeSomeSomeTuple() {
    final Option<Option<Tuple2<String, Integer>>> tuple2OptionOption =
        Option.of(Option.of(Tuple.of("Test", 123)));
    final Some<Tuple2<String, Integer>> actual =
        Match(tuple2OptionOption)
            .of(
                Case(
                    $Some($Some($(Tuple.of("Test", 123)))),
                    value -> {
                      @SuppressWarnings("UnnecessaryLocalVariable")
                      final Some<Tuple2<String, Integer>> some =
                          value; // types are inferred correctly!
                      return some;
                    }));
  ...
  public void shouldDecomposeEmptyList() {
    ...
    final boolean isEmpty =
        Match(list).of(Case($Cons($(), $()), (x, xs) -> false), Case($Nil(), true));
  ...
  public void shouldDecomposeNonEmptyList() {
    ...
    final boolean isNotEmpty =
        Match(list).of(Case($Nil(), false), Case($Cons($(), $()), (x, xs) -> true));
  ...
  public void shouldDecomposeListOfTuple3() {
    final List<Tuple3<String, Integer, Double>> tuple3List =
        List.of(Tuple.of("begin", 10, 4.5), Tuple.of("middle", 11, 0.0), Tuple.of("end", 12, 1.2));
    final String actual =
        Match(tuple3List)
            .of(
                Case(
  ...
  */
  ...
  public void shouldDecomposeSet() {
    ...
    final Set<String> result =
        Match(abc)
            .of( // Does not compile: the Java inference engine sees abc as a Function1<String,
                // Boolean> before a Set<String> thus expects result to be of type Boolean
                Case($(), () -> abc));
  ...
  public void shouldDecomposeValid() {
    ...
    final String actual =
        Match(valid).of(Case($Valid($(1)), i -> "ok"), Case($Invalid($()), error -> error));
  ...
  public void shouldDecomposeInvalid() {
    ...
    final String actual =
        Match(valid).of(Case($Valid($()), i -> "error"), Case($Invalid($("ok")), error -> error));
    assertThat(actual).isEqualTo("ok");
  }
  ...
  public void shouldNotAmbiguous() {
    ...
    { // predicate as variable
      ...
      assertThat(Case($(p), o -> "ok").apply("1"))
          .isEqualTo("ok"); // ambiguous, if Case(T, Function<T, R>) present
      assertThat(Case($(p), () -> "ok").apply("1")).isEqualTo("ok");
      assertThat(Case($(p), "ok").apply("1")).isEqualTo("ok");
    }

    { // $(predicate)
      assertThat(Case($(o -> true), o -> "ok").apply("1"))
          .isEqualTo("ok"); // ambiguous, if Case(T, Function<T, R>) present
    ...
    { // $(value)
      assertThat(Case($("1"), o -> "ok").apply("1"))
          .isEqualTo("ok"); // ambiguous, if Case(T, Function<T, R>) present
      assertThat(Case($("1"), () -> "ok").apply("1")).isEqualTo("ok");
      assertThat(Case($("1"), "ok").apply("1")).isEqualTo("ok");
    }
