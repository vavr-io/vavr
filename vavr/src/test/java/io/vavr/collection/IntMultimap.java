...
public final class IntMultimap<T> implements Traversable<T>, Serializable {
  ...
  // DEV-NOTE: needs to be used internally to ensure the isSameAs property of the original is
  // reflected by this impl
  ...
  public <R> Seq<R> collect(@NonNull PartialFunction<? super T, ? extends R> partialFunction) {
    ...
    final PartialFunction<Tuple2<Integer, T>, R> pf =
        new PartialFunction<Tuple2<Integer, T>, R>() {
          private static final long serialVersionUID = 1L;
  ...
  @Override
  public <C> Map<C, ? extends IntMultimap<T>> groupBy(
      @NonNull Function<? super T, ? extends C> classifier) {
    return original
        .groupBy(e -> classifier.apply(e._2))
        .map((k, v) -> Tuple.of(k, IntMultimap.of(v)));
  }
  ...
  public IntMultimap<T> orElse(@NonNull Supplier<? extends Iterable<? extends T>> supplier) {
    return unit(
        original.orElse(
            () ->
                (Iterable<? extends Tuple2<Integer, T>>)
                    List.ofAll(supplier.get()).zipWithIndex().map(t -> Tuple.of(t._2, t._1))));
  }
  ...
  public IntMultimap<T> replace(T currentElement, T newElement) {
    final Option<Tuple2<Integer, T>> currentEntryOpt =
  ...
  @Override
  public Tuple2<? extends IntMultimap<T>, ? extends IntMultimap<T>> span(
  ...
  @Override
  public <T1, T2, T3> Tuple3<Seq<T1>, Seq<T2>, Seq<T3>> unzip3(
