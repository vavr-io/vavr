...
public class Euler55Test {
   ...
   *
   * <p>Although no one has proved it yet, it is thought that some numbers, like 196, never produce
   ...
   *
   * <p>Surprisingly, there are palindromic numbers that are themselves Lychrel numbers; the first
   * example is 4994.
   *
   * <p>How many Lychrel numbers are there below ten-thousand?
   *
   * <p>See also <a href="https://projecteuler.net/problem=55">projecteuler.net problem 55</a>.
   */
  @Test
  ...
  private static int solve() {
    return Stream.range(1, 10_000).filter(Euler55Test::isLychrel).length();
  }
  ...
  private static String next(String s) {
    return List.of(s, Utils.reverse(s)).map(BigInteger::new).reduce(BigInteger::add).toString();
  }
}
