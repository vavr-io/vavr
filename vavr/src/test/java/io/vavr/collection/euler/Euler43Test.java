...
public class Euler43Test {
   ...
   *
   * <p>The number, 1406357289, is a 0 to 9 pandigital number because it is made up of each of the
   * digits 0 to 9 in some order, but it also has a rather interesting sub-string divisibility
   * property.
   *
   * <p>Let <i>d</i><sub>1</sub> be the 1<sup>st</sup> digit, <i>d</i><sub>2</sub> be the
   ...
   *
   * <p>See also <a href="https://projecteuler.net/problem=43">projecteuler.net problem 43</a>.
   */
  @Test
  ...
  private static Seq<Long> tenDigitPandigitalsWithProperty() {
    ...
    return ALL_DIGITS
        ...
        .map(
            tailDigitsWithProperty ->
                tailDigitsWithProperty.prepend(ALL_DIGITS.removeAll(tailDigitsWithProperty).head()))
