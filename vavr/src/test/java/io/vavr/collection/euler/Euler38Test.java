...
public class Euler38Test {
   ...
   * </pre>
   *
   * <p>By concatenating each product we get the 1 to 9 pandigital, 192384576. We will call
   ...
   *
   * <p>See also <a href="https://projecteuler.net/problem=38">projecteuler.net problem 38</a>.
   */
  @Test
  ...
  private static CharSeq largest1To9PandigitalMultiple() {
    return CharSeq.of("87654321")
        ...
        .sorted()
        .reverse()
        .map(i -> "9" + i) // Since 918273645 is known we don't have to investigate numbers not
        // starting with a 9.
        .map(CharSeq::of)
  ...
  private static boolean isPandigitalMultiple(CharSeq pandigital) {
    return List.rangeClosed(1, pandigital.length() - 1)
        .exists(
            i ->
                isPandigitalMultipleRest(
                    pandigital.drop(i), Integer.valueOf(pandigital.take(i).mkString()), 2));
  }

  private static boolean isPandigitalMultipleRest(
      CharSeq pandigitalRest, int multiplicand, int multiplicator) {
    return Match(pandigitalRest.length())
        .of(
            Case($(0), true),
            Case(
                $(),
                length ->
                    List.rangeClosed(1, length)
                        .find(
                            i ->
                                Integer.valueOf(pandigitalRest.take(i).mkString())
                                    == multiplicand * multiplicator)
                        .map(
                            i ->
                                isPandigitalMultipleRest(
                                    pandigitalRest.drop(i), multiplicand, multiplicator + 1))
                        .getOrElse(false)));
  }
}
