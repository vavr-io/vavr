...
public class Euler33Test {
   ...
   *
   * <p>There are exactly four non-trivial examples of this type of fraction, less than one in
   * value, and containing two digits in the numerator and denominator.
   *
   * <p>If the product of these four fractions is given in its lowest common terms, find the value
   * of the denominator.
   *
   * <p>See also <a href="https://projecteuler.net/problem=33">projecteuler.net problem 33</a>.
   */
  @Test
  public void shouldSolveProblem33() {
    ...
    assertThat(lowestCommonDenominatorOfProductOfTheFourNonTrivialdigitCancellingFractions())
        .isEqualTo(100);
  }
  ...
  private static int lowestCommonDenominatorOfProductOfTheFourNonTrivialdigitCancellingFractions() {
    return List.rangeClosed(10, 98)
        ...
        .apply(Euler33Test::simplifyFraction)
        ._2;
  }
  ...
  private static boolean fractionCanBeSimplifiedByCancellingDigit(
      Tuple2<Integer, Integer> fraction, char d) {
    return Tuple.of(
            CharSeq.of(fraction._1.toString()).remove(d),
            CharSeq.of(fraction._2.toString()).remove(d))
        .map(CharSeq::mkString, CharSeq::mkString)
        .map(Double::valueOf, Double::valueOf)
        .apply((d1, d2) -> fraction._1 / d1 == fraction._2 / d2);
  }

  private static Tuple2<Integer, Integer> multiplyFractions(
      Tuple2<Integer, Integer> f1, Tuple2<Integer, Integer> f2) {
