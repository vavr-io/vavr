...
public class BitSetTest extends AbstractSortedSetTest {
  ...
  private static final int MAX_BIT = 1_000_000;
  ...
  @Override
  protected <T extends Comparable<? super T>> BitSet<T> ofJavaStream(
  ...
  public void shouldCollectInts() {
    final Traversable<Integer> actual =
        java.util.stream.Stream.of(1, 2, 3).collect(BitSet.collector());
  ...
  public void shouldBehaveExactlyLikeAnotherBitSet() {
    for (int i = 0; i < 10; i++) {
      ...
      final Collection<Integer> oldValues =
          mutableBitSet.stream().sorted().boxed().collect(toList());
