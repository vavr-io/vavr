...
public class QuickSortTest {
  ...
  public void shouldQuickSort() {
    final List<Integer> values =
        List(3, 1, 4, 1, 5, 9, 2, 6, 5, 3, 5, 8, 9, 7, 9, 3, 2, 3, 8, 4, 6, 2, 6);
    Assertions.assertThat(sort(values.shuffle())).isEqualTo(values.sorted());
    assertThat(sort2(values.shuffle())).isEqualTo(values.sorted());
  }

  /**
   * Note: this example is only meant to show off, not to be used in reality: it can have quadratic
   * performance and cause stack overflow
   */
