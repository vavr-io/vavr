...
public class VectorPropertyTest {
  ...
  public void shouldCreateAndGet() {
    for (int i = 0; i < 500; i++) {
      ...
      final Vector<Boolean> actualBoolean =
          Vector.ofAll(ArrayType.<boolean[]>asPrimitives(boolean.class, expectedBoolean));
      ...
      final Vector<Byte> actualByte =
          Vector.ofAll(ArrayType.<byte[]>asPrimitives(byte.class, expectedByte));
      ...
      final Vector<Character> actualChar =
          Vector.ofAll(ArrayType.<char[]>asPrimitives(char.class, expectedChar));
      ...
      final Vector<Double> actualDouble =
          Vector.ofAll(ArrayType.<double[]>asPrimitives(double.class, expectedDouble));
      ...
      final Vector<Float> actualFloat =
          Vector.ofAll(ArrayType.<float[]>asPrimitives(float.class, expectedFloat));
      ...
      /* int */
      final Vector<Integer> actualInt =
          Vector.ofAll(ArrayType.<int[]>asPrimitives(int.class, expected));
      ...
      final Vector<Long> actualLong =
          Vector.ofAll(ArrayType.<long[]>asPrimitives(long.class, expectedLong));
      ...
      final Vector<Short> actualShort =
          Vector.ofAll(ArrayType.<short[]>asPrimitives(short.class, expectedShort));
  ...
  public void shouldIterate() {
    ...
    for (int drop = 0; drop <= (BitMappedTrie.BRANCHING_FACTOR + 1); drop++) {
      ...
      actual.trie.<int[]>visit(
          (index, leaf, start, end) -> {
    ...
    for (int i = 1; i < 10; i++) {
      ...
      for (int j = 0; j < 20_000; j++) {
        ...
        if (percent(random) < 20) {
          ...
          actual =
              (percent(random) < 30)
                  ? Vector.narrow(Vector.ofAll(ArrayType.<int[]>asPrimitives(int.class, expected)))
        ...
        if (percent(random) < 10) {
          Iterable<Object> values =
              randomValues(random, random.nextInt(2 * BitMappedTrie.BRANCHING_FACTOR));
        ...
        if (percent(random) < 10) {
          ...
          values =
              (percent(random) < 50) ? Iterator.ofAll(values) : values; /* not traversable again */
        ...
        if (percent(random) < 20) {
          final Function<Object, Object> mapper =
              val -> (val instanceof Integer) ? ((Integer) val + 1) : val;
        ...
        history.forEach(
            t -> assertAreEqual(t._1, t._2)); // test that the modifications are persistent
      }
    }
  }
  ...
  private static <T extends Seq<?>, P> T assertAreEqual(
      T previousActual, P param, Function2<T, P, T> actualProvider, Seq<?> expected) {
    final T actual = actualProvider.apply(previousActual, param);
    assertAreEqual(expected, actual);
    return actual; // makes debugging a lot easier, as the frame can be dropped and rerun on
    // AssertError
  }
