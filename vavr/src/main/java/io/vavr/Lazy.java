 ...
 * {@code Lazy} is memoizing: the computation is performed at most once, ensuring referential
 * transparency.
 *
 * <p>This type behaves more like a <em>Functor</em> than a <em>Monad</em>: it represents a value
 * rather than capturing a specific state. Therefore, it does not provide operations like {@code
 * flatMap} or {@code orElse}.
 *
 * <p>Example usage:
 *
 * <pre>{@code
 ...
 * }</pre>
 *
 * <p>Creating a <em>truly lazy</em> value for an interface type:
 *
 * <pre>{@code
...
public final class Lazy<T> implements Value<T>, Supplier<T>, Serializable {
  ...
  /**
   * Narrows a {@code Lazy<? extends T>} to {@code Lazy<T>} via a type-safe cast. Safe here because
   * the lazy value is immutable and no elements can be added that would violate the type
   * (covariance)
   *
  ...
  /**
   * Creates a {@code Lazy} instance that obtains its value from the given {@code Supplier}. The
   * supplier is invoked at most once, and the result is cached for subsequent calls.
   *
  ...
  /**
   * Combines multiple {@code Lazy} instances into a single {@code Lazy} containing a sequence of
   * their evaluated values.
   *
   * <p>Transforms an {@code Iterable<Lazy<? extends T>>} into a {@code Lazy<Seq<T>>}, evaluating
   * each value lazily when the resulting {@code Lazy} is accessed.
   *
  ...
  /**
   * Creates a true <em>lazy value</em> of type {@code T}, implemented using a {@linkplain
   * java.lang.reflect.Proxy} that delegates to a {@code Lazy} instance.
   *
  ...
  /**
   * Evaluates this lazy value on the first call and caches the result. Subsequent calls return the
   ...
   *
   * <p>Note: The value is evaluated internally (at most once) when {@link #get()} is called.
   *
  ...
  /**
   * Applies a transformation function to the value contained in this {@code Lazy}, producing a new
   * {@code Lazy} instance of the transformed value.
   *
