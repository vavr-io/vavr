...
final class FutureImpl<T> implements Future<T> {
  ...
  /**
   * The queue of actions is filled when calling onComplete() before the Future is completed or
   * cancelled. Otherwise actions = null.
   *
   * <p>GuardedBy("lock")
   */
  ...
  /**
   * The queue of waiters is filled when calling await() before the Future is completed or
   * cancelled. Otherwise waiters = null.
   *
   * <p>GuardedBy("lock")
   */
  ...
  /**
   * Creates a {@code FutureImpl} that needs to be automatically completed by calling {@link
   * #tryComplete(Try)}.
   *
   * @param executor An {@link Executor} to run and control the computation and to perform the
   *     actions.
  ...
  /**
   * Creates a {@code FutureImpl} that is immediately completed with the given value. No task will
   * be started.
   *
   * @param executor An {@link Executor} to run and control the computation and to perform the
   *     actions.
  ...
  /**
   * Creates a {@code FutureImpl} that is eventually completed. The given {@code computation} is
   * <em>synchronously</em> executed, no thread is started.
   *
   * @param executor An {@link Executor} to run and control the computation and to perform the
   *     actions.
  ...
  static <T> FutureImpl<T> sync(Executor executor, Task<? extends T> task) {
    return new FutureImpl<>(
        ...
        Queue.empty(),
        Queue.empty(),
        (Task.SyncTask<T>) complete -> task.run(complete::with));
  }

  /**
   * Creates a {@code FutureImpl} that is eventually completed. The given {@code computation} is
   * <em>asynchronously</em> executed, a new thread is started.
   *
   * @param executor An {@link Executor} to run and control the computation and to perform the
   *     actions.
  ...
  static <T> FutureImpl<T> async(Executor executor, Task<? extends T> task) {
    // In a single-threaded context this Future may already have been completed during
    // initialization.
   ...
   *
   * <p>If timeout = 0 then {@code LockSupport.park()} is called (start, timeout and unit are not
   * used), otherwise {@code LockSupport.park(timeout, unit}} is called.
   *
   * <p>If a timeout > -1 is specified and the deadline is not met, this Future fails with a {@link
   * TimeoutException}.
   *
   * <p>If this Thread was interrupted, this Future fails with a {@link InterruptedException}.
   *
    ...
    try {
      ForkJoinPool.managedBlock(
          new ForkJoinPool.ManagedBlocker() {
             ...
             *
             * <p>LockSupport.park() / parkNanos() may return when the Thread is permitted to be
             * scheduled again. If so, the Future's tryComplete() method wasn't called yet. In that
             * case the block() method is called again. The remaining timeout is recalculated
             * accordingly.
             *
                    ...
                    tryComplete(
                        Try.failure(
                            new TimeoutException(
                                "timeout after " + timeout + " " + unit.name().toLowerCase())));
                  }
                } else {
                  LockSupport.park();
                }
  ...
  private void updateThread() {
    // cancellation may have been initiated by a different thread before this.thread is set by the
    // worker thread
    if (!isCompleted()) {
      lock.lock();
      try {
        if (!isCompleted()) {
          ...
          try {
            ...
            // we are not allowed to set the uncaught exception handler of the worker thread
            // ¯\_(ツ)_/¯
          }
        }
      } finally {
   ...
   *
   * <p>Completes this Future with a value and performs all actions.
   *
   * <p>This method is idempotent. I.e. it does nothing, if this Future is already completed.
   *
