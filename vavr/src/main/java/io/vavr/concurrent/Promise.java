...
/**
 * A {@code Promise} is a write-once container for a read-only {@code Future}, allowing the
 * underlying {@code Future} to be completed with a value or an exception.
 *
 * <p>The associated {@code Executor} is used to run asynchronous handlers, for example, via {@code
 * promise.future().onComplete(...)}.
 *
 * <h2>Creation</h2>
 *
 * <p>{@code Promise} provides static factory methods to create new promises:
 *
 * <ul>
 *   <li>Unfulfilled promises: {@link Promise#make()}
 *   <li>Already completed promises:
 *       <ul>
 *         <li>{@link #failed(Throwable)}
 *         <li>{@link #fromTry(Try)}
 *         <li>{@link #successful(Object)}
 *       </ul>
 * </ul>
 *
 * All factory methods also have variants that accept an {@link Executor}, allowing finer control
 * over thread usage and thread pool configuration.
 *
 * <h3>One-shot API</h3>
 *
 * <p>When a single {@code Thread} is responsible for completing the {@code Promise}, use one of the
 * following methods. Calls will throw an exception if the {@code Promise} has already been
 * completed:
 *
 * <ul>
 ...
 *
 * <p>When multiple {@code Thread}s may attempt to complete the {@code Promise}, use one of the
 * following "try" methods. Calls will return {@code false} if the {@code Promise} is already
 * completed:
 *
 * <ul>
 *   <li>{@link #tryComplete(Try)}
 *   <li>{@link #tryCompleteWith(Future)}
 *   <li>{@link #tryFailure(Throwable)}
 *   <li>{@link #trySuccess(Object)}
 * </ul>
 *
...
public interface Promise<T> {
  ...
  /**
   * Creates a {@code Promise} that is already completed with a failure, using the {@link
  ...
  /**
   * Creates a {@code Promise} that is already completed with a failure, using the specified {@link
  ...
  /**
   * Creates a {@code Promise} from the given {@link Try}, using the specified {@link Executor} for
  ...
  /**
   * Creates a new {@code Promise} that is not yet completed, using Virtual Threads for asynchronous
   * operations.
   *
  ...
  /**
   * Narrows a {@code Promise<? extends T>} to {@code Promise<T>} through a type-safe cast. This is
   * safe because immutable or read-only collections are covariant.
   *
  ...
  /**
   * Creates a {@code Promise} that is already completed successfully, using the {@link
   * Future#DEFAULT_EXECUTOR} for asynchronous operations.
   *
  ...
  /**
   * Creates a {@code Promise} that is already completed successfully, using the specified {@link
   * Executor} for asynchronous operations.
   *
   * @param executor the {@code Executor} used by the underlying {@link Future}
   * @param result the value of the successful result
   ...
   *
   * <p>THE DEFAULT IMPLEMENTATION (obtained by one of the {@link Promise} factory methods) MIGHT
   * THROW AN {@link UnsupportedOperationException} AT RUNTIME, DEPENDING ON WHAT {@link
   * Future#executorService()} returns.
   *
   * @return (never)
   ...
   *
   * @param value a {@link Try.Success} containing the result or a {@link Try.Failure} containing an
   *     exception
   ...
   * @param value a {@link Try.Success} containing the result or a {@link Try.Failure} containing an
   *     exception
   * @return {@code true} if the {@code Promise} was completed successfully, {@code false} if it was
   *     already completed
   */
  boolean tryComplete(@NonNull Try<? extends T> value);

  /**
   * Completes this {@code Promise} with the result of the given {@code Future} once it is
   * completed.
   *
  ...
  /**
   * Attempts to complete this {@code Promise} with the result of the given {@code Future} once it
   * is completed.
   *
   * @param other the {@code Future} whose result or failure may complete this {@code Promise}
   * @return {@code true} if this {@code Promise} was completed by {@code other}, {@code false} if
...
final class PromiseImpl<T> implements Promise<T> {
  ...
  // The underlying FutureImpl is MUTABLE and therefore we CANNOT CHANGE DEFAULT equals() and
  // hashCode() behavior.
