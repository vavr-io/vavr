...
/**
 * Represents the result of an asynchronous computation that becomes available at some point in the
 * future. All operations provided by this {@code Future} are non-blocking.
 *
 * <p>The underlying {@link java.util.concurrent.Executor} is used to execute asynchronous handlers,
 * for example via {@code onComplete(...)}.
 *
 * <p>A {@code Future} has two states:
 *
 * <ul>
 *   <li><strong>Pending:</strong> The computation is ongoing. Only a pending future may be
 *       completed or cancelled.
 *   <li><strong>Completed:</strong> The computation has finished, either successfully with a
 *       result, with an exception, or via cancellation.
 * </ul>
 *
 * <p>Callbacks may be registered on a {@code Future} at any time. These callbacks are executed as
 * soon as the {@code Future} completes. If a callback is registered on a completed {@code Future},
 * it is executed immediately. Execution may occur on a separate thread, depending on the underlying
 * {@code Executor}. Callbacks registered on a cancelled {@code Future} are executed with the failed
 * result.
 *
...
public interface Future<T> extends Value<T> {
  ...
  /**
   * Creates a failed {@code Future} with the given {@code exception}, using the {@link
   * #DEFAULT_EXECUTOR} to execute callbacks.
   *
  ...
  /**
   * Creates a failed {@code Future} with the given {@code exception}, executing callbacks using the
  ...
  /**
   * Returns a {@code Future} that completes with the first result of the given {@code futures} that
   * satisfies the specified {@code predicate}. If no result matches, the {@code Future} will
   * contain {@link Option.None}.
   *
   * <p>The returned {@code Future} uses the {@link #DEFAULT_EXECUTOR} to execute callbacks.
   *
   ...
   * @return a {@code Future} of an {@link Option} containing the first matching result, or {@link
   *     Option.None} if none match
   * @throws NullPointerException if {@code futures} or {@code predicate} is null
   */
  static <T> Future<Option<T>> find(
      @NonNull Iterable<? extends Future<? extends T>> futures,
   ...
   * contain {@link Option.None}.
   *
   * <p>The returned {@code Future} executes using the provided {@link Executor}.
   *
   * @param executor the {@link Executor} to run asynchronous handlers
   * @param futures an iterable of futures to inspect
   * @param predicate a predicate to test successful future results
                ...
                future ->
                    future.onComplete(
                        result -> {
                          lock.lock();
                          try {
                            // if the future is already completed we already found our result and
                            // there is nothing more to do.
                            if (!completed.get()) {
                              // when there are no more results we return a None
                              final boolean wasLast = count.decrementAndGet() == 0;
                              // when result is a Failure or predicate is false then we check in
                              // onFailure for finish
                              result
                                  .filter(predicate)
                                  .onSuccess(
                                      value ->
                                          completed.set(
                                              complete.with(Try.success(Option.some(value)))))
                                  .onFailure(
                                      ignored -> {
                                        if (wasLast) {
  ...
  /**
   * Returns a new {@code Future} that completes with the result of the first future in the given
   * iterable that completes, using the {@link #DEFAULT_EXECUTOR}.
   *
  ...
  /**
   * Returns a new {@code Future} that completes with the result of the first future in the given
   * iterable that completes, using the specified {@link Executor}.
   *
   * @param executor the {@link Executor} to run asynchronous handlers
   * @param futures an iterable of futures to observe
   * @param <T> the type of the future results
  ...
  static <T> Future<T> firstCompletedOf(
      @NonNull Executor executor, @NonNull Iterable<? extends Future<? extends T>> futures) {
    Objects.requireNonNull(executor, "executor is null");
    Objects.requireNonNull(futures, "futures is null");
  ...
  /**
   * Returns a {@code Future} that contains the result of folding the given future values. If any
   * future fails or the fold operation throws an exception, the resulting {@code Future} will also
   * fail.
   *
   * <p>The resulting {@code Future} executes using the {@link #DEFAULT_EXECUTOR}.
   *
  ...
  /**
   * Returns a {@code Future} containing the result of folding the given future values. If any
   * future fails or the fold operation throws an exception, the resulting {@code Future} will also
   * fail.
   *
   * <p>The resulting {@code Future} executes using the specified {@link Executor}.
   *
   * @param executor the {@link Executor} to run asynchronous handlers
   * @param futures an iterable of futures to fold
   * @param zero the initial value for the fold
  ...
  /**
   * Creates a {@code Future} that wraps the given {@link java.util.concurrent.Future}, executing
   * callbacks on the {@link #DEFAULT_EXECUTOR}.
   *
  ...
  /**
   * Creates a {@code Future} that wraps the given {@link java.util.concurrent.Future}, executing
   * callbacks using the specified {@link Executor}.
   *
   * @param executor the {@link Executor} to run asynchronous handlers
   * @param future the {@link java.util.concurrent.Future} to wrap
   * @param <T> the result type of the future
   ...
   */
  static <T> Future<T> fromJavaFuture(
      @NonNull Executor executor, java.util.concurrent.@NonNull Future<T> future) {
    Objects.requireNonNull(executor, "executor is null");
    Objects.requireNonNull(future, "future is null");
   ...
   * @param <T> the result type of the future
   * @return a new {@code Future} containing the result of the given {@link
   ...
   * @throws NullPointerException if {@code executor} or {@code future} is null
   */
  static <T> Future<T> fromCompletableFuture(
      @NonNull Executor executor, @NonNull CompletableFuture<T> future) {
    Objects.requireNonNull(executor, "executor is null");
    Objects.requireNonNull(future, "future is null");
    if (future.isDone() || future.isCompletedExceptionally() || future.isCancelled()) {
      ...
      return run(
          ...
          complete ->
              future.handle(
                  (t, err) -> complete.with((err == null) ? Try.success(t) : Try.failure(err))));
    }
  }
  ...
  /**
   * Creates a {@code Future} from a {@link Try}, executing callbacks using the specified {@link
   * Executor}.
   *
   * @param executor the {@link Executor} to run asynchronous handlers
   * @param result the {@link Try} result to wrap
  ...
  /**
   * Narrows a {@code Future<? extends T>} to {@code Future<T>} via a type-safe cast. This is safe
   * because immutable or read-only collections are covariant.
   *
   ...
   */
  static <T> Future<T> of(
      @NonNull Executor executor, @NonNull CheckedFunction0<? extends T> computation) {
    Objects.requireNonNull(executor, "executor is null");
   ...
   * A common use-case is to hand over the {@code complete} predicate to another {@code Future} in
   ...
   * }</pre>
   *
   * The computation receives a {@link Predicate}, named {@code complete} by convention, that takes
   * a result of type {@code Try<T>} and returns a boolean that states whether the Future was
   * completed.
   *
   * <p>Future completion is an idempotent operation in the way that the first call of {@code
   * complete} will return true, successive calls will return false.
   *
  ...
  /**
   * Returns a {@code Future} containing the result of reducing the given future values. The first
   * completed future serves as the initial (zero) value. If any future or the reduce operation
   * fails, the resulting {@code Future} will also fail.
   *
   * <p>The resulting {@code Future} executes using the {@link #DEFAULT_EXECUTOR}.
   *
   ...
   * @throws NullPointerException if {@code futures} or {@code f} is null
   */
  static <T> Future<T> reduce(
      @NonNull Iterable<? extends Future<? extends T>> futures,
      @NonNull BiFunction<? super T, ? super T, ? extends T> f) {
   ...
   *
   * <ul>
   *   <li>If all given futures succeed, the resulting future also succeeds:
   *       <pre>{@code
   ...
   *
   * }</pre>
   *   <li>If any given future fails, the resulting future fails as well:
   *       <pre>{@code
   ...
   * @throws NullPointerException if {@code executor} or {@code futures} is null
   */
  static <T> Future<Seq<T>> sequence(
      @NonNull Executor executor, @NonNull Iterable<? extends Future<? extends T>> futures) {
    Objects.requireNonNull(executor, "executor is null");
    Objects.requireNonNull(futures, "futures is null");
  ...
  /**
   * Creates a succeeded {@code Future} with the given result, executing callbacks using the
   * specified {@link Executor}.
   *
   * @param executor the {@link Executor} to run asynchronous handlers
   * @param result the successful result
   ...
   * {@code Future} by transforming an {@code Iterable<? extends T>} into a {@code Future<Seq<U>>}.
   *
   * <p>The resulting {@code Future} executes using the {@link #DEFAULT_EXECUTOR}.
   *
  ...
  static <T, U> Future<Seq<U>> traverse(
      ...
      @NonNull Function<? super T, ? extends Future<? extends U>> mapper) {
   ...
   *
   * <p>Exceptions thrown by the given {@code action} are not propagated. Subsequent actions are
   * executed based on the value of the original {@code Future}.
   *
   * <p>Example:
   *
   * <pre>{@code
   ...
   * @return a new {@code Future} containing the original result, completed after the action
   *     executes
  ...
  /**
   * Blocks the current thread until this {@code Future} is completed, or returns immediately if it
   * is already completed.
   *
   * <p>If the current thread is interrupted while waiting, a failed {@code Future} is returned
   * containing the corresponding {@link InterruptedException}.
   *
   ...
   * containing the corresponding {@link InterruptedException}.
   *
   * <p>If the specified timeout is reached before completion, a failed {@code Future} is returned
   * containing a {@link TimeoutException}.
   *
   ...
   *
   * <p>If the future is successfully cancelled, its result becomes a {@code
   * Failure(CancellationException)}.
   *
   * @return {@code false} if this {@code Future} is already completed or could not be cancelled,
   *     {@code true} otherwise
  ...
  /**
   * Cancels this {@code Future}. A pending future may be interrupted depending on the underlying
   * {@code Executor}.
   *
   * <p>If the future is successfully cancelled, its result becomes a {@code
   * Failure(CancellationException)}.
   *
   * @param mayInterruptIfRunning {@code true} if a running thread should be interrupted; {@code
   *     false} allows it to complete
   * @return {@code false} if this {@code Future} is already completed or could not be cancelled,
   *     {@code true} otherwise
   * @throws SecurityException if the current thread is not permitted to modify the Future's thread
   * @see Future#isCancelled()
  ...
  /**
   * Applies a {@code partialFunction} to the value of this {@code Future}, collecting results only
   * for values where the function is defined. The mapped result is wrapped in a new {@code Future}.
   *
   * <p>Example:
   *
   * <pre>{@code
  ...
  default <R> Future<R> collect(@NonNull PartialFunction<? super T, ? extends R> partialFunction) {
    ...
    return run(
        executor(),
        complete -> onComplete(result -> complete.with(result.collect(partialFunction))));
  }
   ...
   *
   * <p>THE DEFAULT IMPLEMENTATION (obtained by one of the {@link Future} factory methods) MIGHT
   * THROW AN {@link UnsupportedOperationException} AT RUNTIME.
   *
   * @return (never)
   * @throws UnsupportedOperationException if the underlying {@link Executor} isn't an {@link
   *     ExecutorService}.
   ...
   *
   * <p>If this {@code Future} succeeds, the resulting failed projection contains a {@code
   * NoSuchElementException}.
   *
   * <p>If this {@code Future} fails, the resulting failed projection succeeds with the exception as
   * its value.
   *
                    ...
                    complete.with(
                        Try.failure(
                            new NoSuchElementException(
                                "Future.failed completed without a throwable")));
                  }
                }));
  }

  /**
   * Returns a {@code Future} that yields the result of this {@code Future} if it succeeds. If this
   * {@code Future} fails, the result of the given {@code that} {@code Future} is returned if it
   * succeeds. If both {@code Future}s fail, the failure of this {@code Future} is returned.
   *
   * <p>Example:
   *
   * <pre>{@code
  ...
  /**
   * Shortcut for {@code filterTry(predicate::test)}, filtering the result of this {@code Future}
   * using the given predicate.
   *
   * @param predicate a predicate to test the value of the future
   * @return a new {@code Future} containing the value if the predicate passes, or a failure
   *     otherwise
  ...
  /**
   * Filters the result of this {@code Future} using the given {@link CheckedPredicate}, delegating
   * to {@link Try#filterTry(CheckedPredicate)}.
   *
  ...
  default Future<T> filterTry(@NonNull CheckedPredicate<? super T> predicate) {
    Objects.requireNonNull(predicate, "predicate is null");
    return run(
        executor(), complete -> onComplete(result -> complete.with(result.filterTry(predicate))));
  }

  /**
   * Returns the underlying exception. This is syntactic sugar for {@code
   * future.getValue().map(Try::getCause)}.
   *
   * @return {@code None} if the {@code Future} is not yet completed, or {@code Some(Throwable)} if
   *     it completed with a failure
   ...
   *
   * @return {@code None} if the {@code Future} is not yet completed or was cancelled; otherwise,
   *     {@code Some(Try)} containing the result or failure
   */
  Option<Try<T>> getValue();

  /**
   * Checks whether this {@code Future} was cancelled, i.e., its computation was interrupted before
   * completion.
   *
  ...
  /**
   * Checks whether this {@code Future} is completed, i.e., whether it has finished with a value,
   ...
   *
   * @return {@code true} if this {@code Future} has completed with a successful result, {@code
   *     false} otherwise
   */
   ...
   *
   * @return {@code true} if this {@code Future} has completed with a failure, {@code false}
   *     otherwise
   */
  ...
  /**
   * Performs the given action once this {@code Future} is complete and its result is a {@link
   * Try.Failure}.
   *
   * <p>Note that a cancelled {@code Future} is also considered a failure.
   *
   ...
   * @return a new {@code Future} containing either the original success or the result of the
   *     recovered {@code Future}
   * @throws NullPointerException if {@code f} is null
   */
  default Future<T> recoverWith(
      @NonNull Function<? super Throwable, ? extends Future<? extends T>> f) {
    Objects.requireNonNull(f, "f is null");
    return run(
        executor(),
        complete ->
  ...
  /**
   * Transforms the value of this {@code Future}, regardless of whether it completed successfully or
   * with a failure.
   *
   ...
   * @throws NullPointerException if {@code f} is null
   */
  default <U> Future<U> transformValue(
      @NonNull Function<? super Try<T>, ? extends Try<? extends U>> f) {
    Objects.requireNonNull(f, "f is null");
    return run(
        executor(),
        complete ->
            onComplete(
                t ->
                    Try.run(() -> complete.with(f.apply(t)))
                        .onFailure(x -> complete.with(Try.failure(x)))));
  }

  /**
   * Combines this {@code Future} with another {@code Future}, returning a {@code Future} of a tuple
   * of both results.
   *
   * <p>If this {@code Future} fails, the resulting {@code Future} contains this failure. Otherwise,
   * it contains the failure of {@code that} {@code Future}, or a tuple of both successful results
   * if both succeed.
   *
   ...
   *
   * <p>If this {@code Future} fails, the resulting {@code Future} contains this failure. Otherwise,
   * it contains the failure of {@code that} {@code Future}, or the result of applying the
   * combinator function to both successful results.
   *
   * @param that another {@code Future} to combine with
   ...
   */
  @SuppressWarnings({"unchecked"})
  default <U, R> Future<R> zipWith(
      @NonNull Future<? extends U> that,
      @NonNull BiFunction<? super T, ? super U, ? extends R> combinator) {
    Objects.requireNonNull(that, "that is null");
                ...
                res1 -> {
                  if (res1.isFailure()) {
                    ...
                    that.onComplete(
                        res2 -> {
                          final Try<R> result =
                              res1.flatMap(t -> res2.map(u -> combinator.apply(t, u)));
  ...
  /**
   * Transforms the value of this {@code Future} using the given {@link Function} if it completes
   * successfully, or returns a {@code Future} with the failure if this {@code Future} fails.
   *
   ...
   * @return a new {@code Future} resulting from applying the mapper, or a {@code Future} with the
   *     failure if this {@code Future} fails
   * @throws NullPointerException if {@code mapper} is {@code null}
   */
  default <U> Future<U> flatMap(
      @NonNull Function<? super T, ? extends Future<? extends U>> mapper) {
    Objects.requireNonNull(mapper, "mapper is null");
  ...
  /**
   * Transforms the value of this {@code Future} using the given {@link CheckedFunction1} if it
   * completes successfully, or returns a {@code Future} with the failure if this {@code Future}
   * fails.
   *
   * <p>If applying the mapper throws an exception, a {@code Future} containing the exception is
   * returned.
   *
   ...
   * @throws NullPointerException if {@code mapper} is {@code null}
   */
  default <U> Future<U> flatMapTry(
      @NonNull CheckedFunction1<? super T, ? extends Future<? extends U>> mapper) {
    Objects.requireNonNull(mapper, "mapper is null");
    return run(
        executor(),
        complete ->
  ...
  /**
   * Performs the given {@code action} asynchronously when this {@code Future} completes
   * successfully.
   *
   * <p>The {@code action} is not executed if the {@code Future} completes with a failure.
   *
  ...
  /**
   * Returns the value of this {@code Future} if it completed successfully, or throws the underlying
   * exception if it completed with a failure. Blocks the current thread if the computation is not
   * yet finished.
   *
   * <p><strong>Note:</strong> If the computation failed, the underlying {@link Throwable} cause is
   * thrown.
   *
  ...
  /**
   * Maps the value of this {@code Future} to a new value using the given {@link CheckedFunction1}
   ...
   * @return a new {@code Future} containing the mapped value if this {@code Future} completes
   *     successfully, otherwise a {@code Future} with the failure
   * @throws NullPointerException if {@code mapper} is {@code null}
   */
  ...
  /**
   * Returns this {@code Future} if it completes successfully, or the given alternative {@code
   * Future} if this {@code Future} fails.
   *
  ...
  /**
   * Returns this {@code Future} if it completes successfully, or a {@code Future} supplied by the
   * given {@link Supplier} if this {@code Future} fails.
   *
   * <p>The supplier is only invoked if this {@code Future} fails.
   *
   * @param supplier a supplier of an alternative {@code Future}
   * @return this {@code Future} if it completes successfully, otherwise the {@code Future} returned
   *     by {@code supplier}
