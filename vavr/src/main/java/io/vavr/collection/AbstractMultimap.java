...
/**
 * An abstract base implementation of the {@link Multimap} interface that provides common
 * functionality for concrete Multimap implementations. This class is not intended to be public and
 * serves as a foundation for specialized Multimap types.
 *
 * @param <K> Key type
 * @param <V> Value type
...
abstract class AbstractMultimap<K, V, M extends Multimap<K, V>> implements Multimap<K, V> {
  ...
  /**
   * Creates a new AbstractMultimap with the specified backing map, container type, and empty
   * container supplier.
   *
  ...
  /**
   * Creates a new Multimap from the given entries by grouping values by their keys. For each key, a
   * new container is created using the emptyContainer supplier, and values are added using the
   * containerType's add operation.
   *
   * @param <K2> Key type of the new multimap
   * @param <V2> Value type of the new multimap
  ...
  private <K2, V2> Multimap<K2, V2> createFromEntries(
      Iterable<? extends Tuple2<? extends K2, ? extends V2>> entries) {
  ...
  @Override
  public <K2, V2> Multimap<K2, V2> bimap(
  ...
  @Override
  public <K2, V2> Multimap<K2, V2> flatMap(
  ...
  @Override
  public <K2, V2> Multimap<K2, V2> map(
      @NonNull BiFunction<? super K, ? super V, Tuple2<K2, V2>> mapper) {
    Objects.requireNonNull(mapper, "mapper is null");
    return foldLeft(
        this.emptyInstance(), (acc, entry) -> acc.put(mapper.apply(entry._1, entry._2)));
  }
  ...
  public M distinct() {
    return (M)
        (containerType == ContainerType.SEQ ? createFromEntries(iterator().distinct()) : this);
  }
  ...
  @SuppressWarnings("unchecked")
  @Override
  public <K2 extends K, V2 extends V> M merge(
    ...
    if (isEmpty()) {
      ...
      final Map<K, Traversable<V>> result =
          that.keySet()
              .foldLeft(
                  this.back,
                  (map, key) -> {
                    final Traversable<V> thisValues =
                        map.get(key).getOrElse((Traversable<V>) emptyContainer.get());
                    final Traversable<V2> thatValues = that.get(key).get();
                    final Traversable<V> newValues =
                        collisionResolution.apply(thisValues, thatValues);
  ...
  /**
   * Returns this {@code Multimap} if it is nonempty, otherwise {@code Multimap} created from
   * iterable, using existing multimap properties.
   *
   * @param other An alternative {@code Traversable}
   * @return this {@code Multimap} if it is nonempty, otherwise {@code Multimap} created from
   *     iterable, using existing multimap properties.
   */
  @SuppressWarnings("unchecked")
  @Override
  ...
  /**
   * Returns this {@code Multimap} if it is nonempty, otherwise {@code Multimap} created from result
   * of evaluating supplier, using existing multimap properties.
   *
   * @param supplier An alternative {@code Traversable}
   * @return this {@code Multimap} if it is nonempty, otherwise {@code Multimap} created from result
   *     of evaluating supplier, using existing multimap properties.
   */
  @SuppressWarnings("unchecked")
  @Override
  ...
  public M replace(@NonNull Tuple2<K, V> currentElement, Tuple2<K, V> newElement) {
    ...
    return (M)
        (containsKey(currentElement._1)
            ? remove(currentElement._1, currentElement._2).put(newElement)
            : this);
  }
  ...
  public M retainAll(@NonNull Iterable<? extends Tuple2<K, V>> elements) {
    ...
    return (M)
        createFromEntries(back.flatMap(t -> t._2.map(v -> Tuple.of(t._1, v))).retainAll(elements));
  }
