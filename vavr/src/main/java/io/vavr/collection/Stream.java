...
/**
 * An immutable {@code Stream} is lazy sequence of elements which may be infinitely long. Its
 * immutability makes it suitable for concurrent programming.
 *
 * <p>A {@code Stream} is composed of a {@code head} element and a lazy evaluated {@code tail}
 * {@code Stream}.
 *
 * <p>There are two implementations of the {@code Stream} interface:
 *
 * <ul>
 *   <li>{@link Empty}, which represents the empty {@code Stream}.
 *   <li>{@link Cons}, which represents a {@code Stream} containing one or more elements.
 * </ul>
 *
...
public interface Stream<T> extends LinearSeq<T> {
   ...
   * java.util.stream.Stream#collect(java.util.stream.Collector)} to obtain a {@link Stream}.
   *
   ...
   *
   * <p>The {@code Stream} extends to {@code Integer.MIN_VALUE} when passing {@code
   * Integer.MAX_VALUE}.
   *
  ...
  /**
   * Returns an infinite long Stream of {@code int} values starting from {@code value} and spaced by
   * {@code step}.
   *
   * <p>The {@code Stream} extends to {@code Integer.MIN_VALUE} when passing {@code
   * Integer.MAX_VALUE}.
   ...
   *
   * <p>The {@code Stream} extends to {@code Integer.MIN_VALUE} when passing {@code Long.MAX_VALUE}.
   *
  ...
  /**
   * Returns an infinite long Stream of {@code long} values starting from {@code value} and spaced
   * by {@code step}.
   *
   * <p>The {@code Stream} extends to {@code Long.MIN_VALUE} when passing {@code Long.MAX_VALUE}.
   *
   * @param value a start long value
   * @param step the step by which to advance on each next value
  ...
  /**
   * Generates a (theoretically) infinitely long Stream using a repeatedly invoked supplier that
   * provides a {@code Some} for each next value and a {@code None} for the end. The {@code
   * Supplier} will be invoked only that many times until it returns {@code None}, and repeated
   * iteration over the stream will produce the same values in the same order, without any further
   * invocations to the {@code Supplier}.
   *
   ...
   * @param tailSupplier A supplier of the tail values. To end the stream, return {@link
   *     Stream#empty}.
   * @param <T> value type
   * @return A new Stream
   */
   ...
   *
   * <p>Note: this method intentionally returns type {@code Stream} and not {@code Empty}. This
   * comes in handy when folding. If you explicitly need type {@code Empty} use {@linkplain
   * Empty#instance()}.
   *
  ...
  /**
   * Narrows a widened {@code Stream<? extends T>} to {@code Stream<T>} by performing a type-safe
  ...
  /**
   * Returns a Stream containing {@code n} values of a given Function {@code f} over a range of
   ...
   * @return A Stream of size {@code n}, where each element contains the result supplied by {@code
   *     s}.
  ...
  /**
   * Creates a Stream of char numbers starting from {@code from}, extending to {@code toExclusive -
   * 1}.
   *
   * <p>Examples:
   *
  ...
  /**
   * Creates a Stream of double numbers starting from {@code from}, extending up to but not
  ...
  /**
   * Creates a Stream of int numbers starting from {@code from}, extending to {@code toExclusive -
   * 1}.
   *
   * <p>Examples:
   *
  ...
  /**
   * Creates a Stream of long numbers starting from {@code from}, extending to {@code toExclusive -
   * 1}.
   *
   * <p>Examples:
   *
   ...
   *     <p>ex: {@code Stream.transpose(Stream(Stream(1,2,3), Stream(4,5,6))) → Stream(Stream(1,4),
   *     Stream(2,5), Stream(3,6)) }
   */
  ...
  /**
   * Creates a Stream from a seed value and a function. The function takes the seed at first. The
   * function should return {@code None} when it's done generating the Stream, otherwise {@code
   ...
   * @throws NullPointerException if {@code f} is null
   */
  static <T, U> Stream<U> unfoldRight(
   ...
   * Creates a Stream from a seed value and a function. The function takes the seed at first. The
   * function should return {@code None} when it's done generating the Stream, otherwise {@code
   * Some} {@code Tuple} of the value to add to the resulting Stream and the element for the next
   * call.
   *
   * <p>Example:
   *
   ...
   * @throws NullPointerException if {@code f} is null
   */
  static <T, U> Stream<U> unfoldLeft(
  ...
  default Stream<T> append(T element) {
    return isEmpty()
        ? Stream.of(element)
        : new AppendElements<>(head(), io.vavr.collection.Queue.of(element), this::tail);
  }
   ...
   *
   * <p>Well known Scala code for Fibonacci infinite sequence
   *
   * <pre>{@code
  ...
  @Override
  default <U> Stream<U> flatMap(
      @NonNull Function<? super T, ? extends Iterable<? extends U>> mapper) {
    Objects.requireNonNull(mapper, "mapper is null");
    return isEmpty()
        ? Empty.instance()
        : Stream.ofAll(new FlatMapIterator<>(this.iterator(), mapper));
  }
  ...
  default int lastIndexOf(T element, int end) {
    ...
    for (Stream<T> stream = this;
        index <= end && !stream.isEmpty();
        stream = stream.tail(), index++) {
  ...
  @Override
  default <U extends Comparable<? super U>> Stream<T> sortBy(
      @NonNull Function<? super T, ? extends U> mapper) {
    return sortBy(U::compareTo, mapper);
  }

  @Override
  default <U> Stream<T> sortBy(
    ...
    for (int i = 0; i < beginIndex; i++, result = result.tail()) {
      if (result.isEmpty()) {
        throw new IndexOutOfBoundsException(
            "subSequence(" + beginIndex + ") on Stream of size " + i);
      }
    }
    return result;
  }
  ...
  @Override
  default <U> Stream<Tuple2<T, U>> zipAll(
      @NonNull Iterable<? extends U> iterable, T thisElem, U thatElem) {
  ...
  @Override
  default <U> Stream<U> zipWithIndex(
   ...
   * @return new {@code Stream} composed from this stream extended with values provided by the
   *     supplier
   */
   ...
   * @return new {@code Stream} composed from this stream extended with values calculated by the
   *     provided function
   */
   ...
   * The empty Stream.
   *
   * <p>This is a singleton, i.e. not Cloneable.
   *
   * @param <T> Component type of the Stream.
   */
...
interface StreamModule {
  ...
  final class AppendElements<T> extends Cons<T> implements Serializable {
    ...
    public Stream<T> appendAll(@NonNull Iterable<? extends T> elements) {
      ...
      return isEmpty()
          ? Stream.ofAll(queue)
          : new AppendElements<>(head, queue.appendAll(elements), tail);
    }
  ...
  /**
   * A serialization proxy which, in this context, is used to deserialize immutable, linked Streams
  ...
  final class AppendSelf<T> {
    ...
    private Cons<T> appendAll(
        Cons<T> stream, Function<? super Stream<T>, ? extends Stream<T>> mapper) {
      return (Cons<T>)
          Stream.cons(
              stream.head(),
              () -> {
  ...
  interface DropRight {
    ...
    static <T> Stream<T> apply(
        io.vavr.collection.List<T> front, io.vavr.collection.List<T> rear, Stream<T> remaining) {
  ...
  final class StreamIterator<T> extends AbstractIterator<T> {
    ...
    public T getNext() {
      ...
      // DEV-NOTE: we make the stream even more lazy because the next head must not be evaluated on
      // hasNext()
      current = stream::tail;
      return stream.head();
    }
