 ...
 * removal.
 *
 * <p>This class is a drop-in alternative behind the {@link HashArrayMappedTrie} interface and does
 * not alter the wiring of {@code HashMap}/{@code HashSet}; it exists so the two representations can
 * be A/B compared for correctness and performance.
 *
...
final class CompressedHashArrayMappedPrefixTrie<K, V> implements HashArrayMappedTrie<K, V>, Serializable {
    ...
    public HashArrayMappedTrie<K, V> remove(K key) {
        ...
        return new CompressedHashArrayMappedPrefixTrie<>(newRoot, size - 1);
    }
    ...
    private static final class BitmapIndexedNode<K, V> extends Node<K, V> {
        ...
        Node<K, V> removed(K key, int keyHash, int shift, MapDetails details) {
            ...
            if ((nodeMap & bitpos) != 0) {
                ...
                switch (subNew.sizePredicate()) {
                    case SIZE_ONE:
                        if (payloadArity() == 0 && nodeArity() == 1) {
                            return subNew; // bubble the singleton up
                        }
                        return copyAndMigrateFromNodeToInline(bitpos, subNew);
                    case SIZE_MORE:
                        return copyAndSetNode(bitpos, subNew);
                    default:
                        throw new IllegalStateException("unexpected empty sub-node after removal");
                }
            }
            return this;
        }
