...
/**
 * An immutable {@code List} is an eager sequence of elements. Its immutability makes it suitable
 * for concurrent programming.
 *
 * <p>A {@code List} is composed of a {@code head} element and a {@code tail} {@code List}.
 *
 * <p>There are two implementations of the {@code List} interface:
 *
 * <ul>
 *   <li>{@link Nil}, which represents the empty {@code List}.
 *   <li>{@link Cons}, which represents a {@code List} containing one or more elements.
 * </ul>
 *
 * A {@code List} is a {@code Stack} in the sense that it stores elements allowing a
 ...
 * }</pre>
 *
 * Note: A {@code List} is primarily a {@code Seq} and extends {@code Stack} for technical reasons
 * (so {@code Stack} does not need to wrap {@code List}).
 *
 * <p>If operating on a {@code List}, please prefer
 *
 * <ul>
 *   <li>{@link #prepend(Object)} over {@link #push(Object)}
 *   <li>{@link #prependAll(Iterable)} over {@link #pushAll(Iterable)}
 *   <li>{@link #tail()} over {@link #pop()}
 *   <li>{@link #tailOption()} over {@link #popOption()}
 * </ul>
 *
...
public interface List<T> extends LinearSeq<T> {
   ...
   * java.util.stream.Stream#collect(java.util.stream.Collector)} to obtain a {@link List}.
   *
   * @param <T> Component type of the List.
   ...
   *
   * <p>Note: this method intentionally returns type {@code List} and not {@code Nil}. This comes in
   * handy when folding. If you explicitly need type {@code Nil} use {@linkplain Nil#instance()}.
   *
  ...
  /**
   * Narrows a widened {@code List<? extends T>} to {@code List<T>} by performing a type-safe cast.
   ...
   * Creates a List of the given elements.
   *
   * <p>The resulting list has the same iteration order as the given iterable of elements if the
  ...
  /**
   * Returns a List containing {@code n} values of a given Function {@code f} over a range of
  ...
  /**
   * Creates a List of char numbers starting from {@code from}, extending to {@code toExclusive -
   * 1}.
   *
   * <p>Examples:
   *
  ...
  /**
   * Creates a List of double numbers starting from {@code from}, extending up to but not including
  ...
  /**
   * Creates a List of long numbers starting from {@code from}, extending to {@code toExclusive -
   * 1}.
   *
   * <p>Examples:
   *
   ...
   *     <p>ex: {@code List.transpose(List(List(1,2,3), List(4,5,6))) → List(List(1,4), List(2,5),
   *     List(3,6)) }
   */
  ...
  /**
   * Creates a list from a seed value and a function. The function takes the seed at first. The
   * function should return {@code None} when it's done generating the list, otherwise {@code Some}
   * {@code Tuple} of the element for the next call and the value to add to the resulting list.
   *
   * <p>Example:
   *
   * <pre>{@code
  ...
  static <T, U> List<U> unfoldRight(
      T seed, @NonNull Function<? super T, @NonNull Option<Tuple2<? extends U, ? extends T>>> f) {
   ...
   * Creates a list from a seed value and a function. The function takes the seed at first. The
   * function should return {@code None} when it's done generating the list, otherwise {@code Some}
   * {@code Tuple} of the value to add to the resulting list and the element for the next call.
   *
   * <p>Example:
   *
   * <pre>{@code
    ...
    for (int i = index; i > 0; i--, tail = tail.tail()) {
      if (tail.isEmpty()) {
        throw new IndexOutOfBoundsException(
            "insert(" + index + ", e) on List of length " + length());
      }
  ...
  default List<T> insertAll(int index, @NonNull Iterable<? extends T> elements) {
    ...
    for (int i = index; i > 0; i--, tail = tail.tail()) {
      if (tail.isEmpty()) {
        throw new IndexOutOfBoundsException(
            "insertAll(" + index + ", elements) on List of length " + length());
      }
      preceding = preceding.prepend(tail.head());
    }
   ...
   * Returns the head element without modifying the List.
   *
   * @return {@code None} if this List is empty, otherwise a {@code Some} containing the head
   *     element
   */
   ...
   * Removes the head element from this List.
   *
   * @return {@code None} if this List is empty, otherwise a {@code Some} containing the elements of
   *     this List without the head element
   */
   ...
   * Removes the head element from this List.
   *
   * @return {@code None} if this List is empty, otherwise {@code Some} {@code Tuple} containing the
   *     head element and the remaining elements of this List
   */
  ...
  /**
   * Pushes the given elements on top of this List. A List has LIFO order, i.e. the last of the
   * given elements is the first which will be retrieved.
   *
  ...
  @Override
  default <U extends Comparable<? super U>> List<T> sortBy(
      @NonNull Function<? super T, ? extends U> mapper) {
    return sortBy(U::compareTo, mapper);
  }

  @Override
  default <U> List<T> sortBy(
  ...
  default List<T> update(int index, T element) {
    ...
    for (int i = index; i > 0; i--, tail = tail.tail()) {
      if (tail.isEmpty()) {
        throw new IndexOutOfBoundsException(
            "update(" + index + ", e) on List of length " + length());
      }
      preceding = preceding.prepend(tail.head());
    }
    if (tail.isEmpty()) {
  ...
  @Override
  default <U> List<Tuple2<T, U>> zipAll(
   ...
   * @param <T> Component type of the List.
   */
  // DEV NOTE: class declared final because of serialization proxy pattern (see Effective Java, 2nd
  // ed., p. 315)
...
interface ListModule {
  ...
  interface Combinations {
    ...
    static <T> List<List<T>> apply(List<T> elements, int k) {
      if (k == 0) {
        ...
        return elements
            .zipWithIndex()
            .flatMap(t -> apply(elements.drop(t._2 + 1), (k - 1)).map(c -> c.prepend(t._1)));
      }
    }
  }
  ...
  interface SplitAt {
    ...
    static <T> Tuple2<List<T>, List<T>> splitByPredicateReversed(
        List<T> source, @NonNull Predicate<? super T> predicate) {
      Objects.requireNonNull(predicate, "predicate is null");
      List<T> init = Nil.instance();
      List<T> tail = source;
      while (!tail.isEmpty() && !predicate.test(tail.head())) {
