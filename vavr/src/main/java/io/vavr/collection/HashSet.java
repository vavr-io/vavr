...
public final class HashSet<T> implements Set<T>, Serializable {
   ...
   * java.util.stream.Stream#collect(java.util.stream.Collector)} to obtain a {@link HashSet}.
   *
  ...
  /**
   * Narrows a {@code HashSet<? extends T>} to {@code HashSet<T>} via a type-safe cast. Safe here
  ...
  /**
   * Returns an HashSet containing {@code n} values of a given Function {@code f} over a range of
   ...
   * @return An HashSet of size {@code n}, where each element contains the result supplied by {@code
   *     s}.
  ...
  /**
   * Creates a HashSet of int numbers starting from {@code from}, extending to {@code toExclusive -
   * 1}.
   *
   * <p>Examples:
   *
  ...
  /**
   * Creates a HashSet of char numbers starting from {@code from}, extending to {@code toExclusive -
   * 1}.
   *
   * <p>Examples:
   *
  ...
  /**
   * Creates a HashSet of double numbers starting from {@code from}, extending to {@code toExclusive
   * - 1}, with {@code step}.
   *
   * <p>Examples:
   *
  ...
  /**
   * Creates a HashSet of long numbers starting from {@code from}, extending to {@code toExclusive -
   * 1}.
   *
   * <p>Examples:
   *
  ...
  /**
   * Creates a HashSet of double numbers starting from {@code from}, extending to {@code
  ...
  @Override
  public <U> HashSet<U> flatMap(
      @NonNull Function<? super T, ? extends Iterable<? extends U>> mapper) {
    Objects.requireNonNull(mapper, "mapper is null");
    if (isEmpty()) {
      return empty();
    } else {
      final HashArrayMappedTrie<U, U> that =
          foldLeft(HashArrayMappedTrie.empty(), (tree, t) -> addAll(tree, mapper.apply(t)));
      return new HashSet<>(that);
    }
  }
  ...
  @Override
  public <U> HashSet<Tuple2<T, U>> zipAll(
  ...
  @Override
  public <U> HashSet<U> zipWithIndex(
  ...
  private static <T> HashArrayMappedTrie<T, T> addAll(
      HashArrayMappedTrie<T, T> initial, Iterable<? extends T> additional) {
