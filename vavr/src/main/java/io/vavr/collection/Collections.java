...
final class Collections {
  ...
  static <T, C extends Seq<T>> C asJava(
      C source, Consumer<? super java.util.List<T>> action, ChangePolicy changePolicy) {
  ...
  static <T, S extends Seq<T>> Iterator<S> crossProduct(S empty, S seq, int power) {
    if (power < 0) {
      ...
      return Iterator.range(0, power)
          .foldLeft(
              Iterator.of(empty),
              (product, ignored) -> product.flatMap(el -> seq.map(t -> (S) el.append(t))));
    }
  }
  ...
  static <C extends Traversable<T>, T> C fill(
  ...
  static <C extends Traversable<T>, T> C fillObject(
      int n, T element, @NonNull C empty, @NonNull Function<T[], C> of) {
    Objects.requireNonNull(empty, "empty is null");
    Objects.requireNonNull(of, "of is null");
    if (n <= 0) {
      return empty;
  ...
  static <T, C, R extends Iterable<T>> Map<C, R> groupBy(
  ...
  private static <T, C> java.util.Set<java.util.Map.Entry<C, Collection<T>>> groupBy(
      Traversable<T> source, Function<? super T, ? extends C> classifier) {
    final java.util.Map<C, Collection<T>> results =
        new java.util.LinkedHashMap<>(source.isTraversableAgain() ? source.size() : 16);
    for (T value : source) {
  ...
  @SuppressWarnings("unchecked")
  static <K, V, K2, U extends Map<K2, V>> U mapKeys(
      Map<K, V> source,
      U zero,
  ...
  static <C extends Traversable<T>, T> Tuple2<C, C> partition(
  ...
  static <T, U, R extends Traversable<U>> R scanRight(
    ...
    return scanLeft(
        ...
        us -> finisher.apply(reverseIterator(us)));
  }

  static <T, U, R extends Seq<T>> R sortBy(
      Seq<? extends T> source,
  ...
  static <C extends Traversable<T>, T> C tabulate(
  ...
  static <T, U extends Seq<T>, V extends Seq<U>> V transpose(
  ...
  private static <T, U extends Seq<T>, V extends Seq<U>> V transposeNonEmptyMatrix(
      V matrix, Function<Iterable<U>, V> rowFactory, Function<T[], U> columnFactory) {
  ...
  static <T> IterableWithSize<T> withSize(Iterable<? extends T> iterable) {
    return isTraversableAgain(iterable)
        ? withSizeTraversable(iterable)
        : withSizeTraversable(List.ofAll(iterable));
  }
