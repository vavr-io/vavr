 ...
 *
 * <p>Indexed sequences provide efficient random access and preserve element order.
 *
...
public interface IndexedSeq<T> extends Seq<T> {
   ...
   *
   * <p>This is safe for immutable, read-only sequences, which are covariant.
   *
  ...
  @Override
  <T1, T2, T3>
      Tuple3<? extends IndexedSeq<T1>, ? extends IndexedSeq<T2>, ? extends IndexedSeq<T3>> unzip3(
  ...
  /**
   * Searches this sequence for a specific element using a binary search. The sequence must already
...
interface IndexedSeqModule {
  ...
  class Slice {
    ...
    private static <T> int findSlice(
        IndexedSeq<T> source, IndexedSeq<T> slice, int index, int maxIndex) {
      while (index <= maxIndex) {
