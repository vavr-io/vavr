 ...
 * Each node has a maximum of `32` children (configurable). Access to a given position is done by
 * converting the index to a base 32 number and using each digit to descend down the tree. Modifying
 * the tree is done similarly, but along the way the path is copied, returning a new root every
 * time. `Append` inserts in the last leaf, or if the tree is full from the right, it adds another
 * layer on top of it (the old root will be the first of the new one). `Prepend` is done similarly,
 * but an offset is needed, because adding a new top node (where the current root would be the last
 * node of the new root) shifts the indices by half of the current tree's full size. The `offset`
 * shifts them back to the correct index. `Slice` is done by trimming the path from the root and
 * discarding any `leading`/`trailing` values in effectively constant time (without memory leak, as
 * in `Java`/`Clojure`).
 *
 * @author Pap Lőrinc
 */
final class BitMappedTrie<T> implements Serializable {
  ...
  private BitMappedTrie<T> append(java.util.Iterator<? extends T> iterator, int size) {
    ...
    while (size > 0) {
      ...
      array =
          result.modify(array, shift, index, COPY_NODE, appendToLeaf(iterator, leafSpace + delta));
  ...
  BitMappedTrie<T> update(int index, T element) {
    try {
      final Object root =
          modify(array, depthShift, offset + index, COPY_NODE, updateLeafWith(type, element));
  ...
  BitMappedTrie<T> take(int n) {
    if (n >= length) {
      ...
      final int index = n - 1;
      final Object root =
          arePointingToSameLeaf(index, length - 1)
              ? array
  ...
  private static <T> BitMappedTrie<T> collapsed(
      ArrayType<T> type, Object array, int offset, int length, int shift) {
  ...
  /**
   * fetch the leaf, corresponding to the given index. Node: the offset and length should be taken
   * into consideration as there may be leading and trailing garbage. Also, the returned array is
   * mutable, but should not be mutated!
   */
  ...
  BitMappedTrie<T> filter(Predicate<? super T> predicate) {
    ...
    final int length =
        this.<T>visit(
            (index, leaf, start, end) -> filter(predicate, results, index, leaf, start, end));
    return (this.length == length) ? this : BitMappedTrie.ofAll(type.copyRange(results, 0, length));
  }

  private int filter(
      Predicate<? super T> predicate, Object results, int index, T leaf, int start, int end) {
