 ...
 *
 * <p>Vavr {@code Set} implementations generally support {@code null} elements, but {@code
 * SortedSet} implementations require a {@link Comparator}, which may not support {@code null}.
 *
 * <p>Supports standard set operations like addition, removal, union, intersection, and difference.
...
public interface Set<T> extends Traversable<T>, Function1<T, Boolean>, Serializable {
  ...
  /**
   * Narrows a widened {@code Set<? extends T>} to {@code Set<T>} by performing a type-safe cast.
  ...
  /**
   * Returns a new set containing all elements of this set plus the given element, if it was not
   * already present.
   *
  ...
  /**
   * Returns a new set containing all elements of this set plus the given elements, excluding
   * duplicates.
   *
   ...
   *
   * <p>This method is equivalent to {@link #contains(Object)}.
   *
  ...
  @Override
  <T1, T2> Tuple2<? extends Set<T1>, ? extends Set<T2>> unzip(
      @NonNull Function<? super T, Tuple2<? extends T1, ? extends T2>> unzipper);

  @Override
  <T1, T2, T3> Tuple3<? extends Set<T1>, ? extends Set<T2>, ? extends Set<T3>> unzip3(
