...
/**
 * Purely functional Red/Black Tree, inspired by <a
 * href="https://github.com/kazu-yamamoto/llrbtree/blob/master/Data/Set/RBTree.hs">Kazu Yamamoto's
 * Haskell implementation</a>.
 *
 * <p>Based on
 *
 * <ul>
 *   <li><a href="http://www.eecs.usma.edu/webs/people/okasaki/pubs.html#jfp99">Chris Okasaki,
 *       "Red-Black Trees in a Functional Setting", Journal of Functional Programming, 9(4), pp
 *       471-477, July 1999</a>
 *   <li>Stefan Kahrs, "Red-black trees with types", Journal of functional programming, 11(04), pp
 *       425-432, July 2001
 * </ul>
 *
...
interface RedBlackTree<T> extends Iterable<T>, Serializable {
   ...
   *
   * @param value A value.
   * @return A new tree if this tree does not contain the given value, otherwise the same tree
   *     instance.
   */
   ...
   *
   * <p>An empty node is {@code BLACK} by definition.
   *
  ...
  /**
   * Finds the value stored in this tree, if exists, by applying the underlying comparator to the
   * tree elements and the given element.
   *
   * <p>Especially the value returned may differ from the given value, even if the underlying
   * comparator states that both are equal.
   *
   * @param value A value
   * @return Some value, if this tree contains a value equal to the given value according to the
   *     underlying comparator. Otherwise None.
   */
  ...
  default RedBlackTree<T> intersection(RedBlackTree<T> tree) {
    ...
    if (isEmpty()) {
      ...
      if (contains(that.value)) {
        return Node.join(
            split._1.intersection(that.left), that.value, split._2.intersection(that.right));
      } else {
   ...
   * @return A new RedBlackTree that contains all distinct elements of this and the given {@code
   *     tree}.
   */
   ...
   *
   * <p>Internally an in-order traversal of the RedBlackTree is performed.
   *
   * <p>Example:
   *
   * <pre>{@code
   ...
   *
   * <p>See also <a
   * href="http://n00tc0d3r.blogspot.de/2013/08/implement-iterator-for-binarytree-i-in.html">Implement
   * Iterator for BinaryTree I (In-order)</a>.
   */
  @Override
    ...
    if (isEmpty()) {
      ...
      return new AbstractIterator<T>() {
        ...
        public T getNext() {
          ...
          stack =
              node.right.isEmpty() ? result._2 : pushLeftChildren(result._2, (Node<T>) node.right);
  ...
  final class Node<T> implements RedBlackTree<T>, Serializable {
    ...
    Node<T> color(Color color) {
      return (this.color == color)
          ? this
          : new Node<>(color, blackHeight, left, value, right, empty);
    }
          ...
          if (ln.color == RED) {
            if (!ln.left.isEmpty()) {
              ...
              if (lln.color == RED) {
                ...
                final Node<T> newRight =
                    new Node<>(BLACK, blackHeight, ln.right, value, right, empty);
            ...
            if (!ln.right.isEmpty()) {
              ...
              if (lrn.color == RED) {
                ...
                final Node<T> newRight =
                    new Node<>(BLACK, blackHeight, lrn.right, value, right, empty);
        ...
        if (!right.isEmpty()) {
          ...
          if (rn.color == RED) {
            ...
            if (!rn.left.isEmpty()) {
              ...
              if (rln.color == RED) {
                final Node<T> newLeft =
                    new Node<>(BLACK, blackHeight, left, value, rln.left, empty);
                final Node<T> newRight =
      ...
      if (tree.isEmpty()) {
        ...
        if (comparison < 0) {
          ...
          if (node.right.isEmpty()) {
            ...
            if (d) {
              return Node.unbalancedLeft(
                  node.color, node.blackHeight - 1, node.left, m, r, node.empty);
            } else {
    ...
    private static <T> Tuple3<? extends RedBlackTree<T>, Boolean, T> deleteMin(Node<T> node) {
      if (node.color() == BLACK && node.left().isEmpty() && node.right.isEmpty()) {
        ...
        if (deleted) {
          final Tuple2<Node<T>, Boolean> tD =
              Node.unbalancedRight(
                  node.color, node.blackHeight - 1, l, node.value, node.right, node.empty);
    ...
    private static <T> Node<T> mergeEQ(Node<T> n1, Node<T> n2) {
      ...
      if (n1.blackHeight == h2) {
        ...
        return new Node<>(
            RED, n1.blackHeight + 1, Node.color(n1.left, BLACK), n1.value, node, n1.empty);
    ...
    private static <T> Tuple2<Node<T>, Boolean> unbalancedLeft(
      ...
      if (!left.isEmpty()) {
        ...
        if (ln.color == BLACK) {
          final Node<T> newNode =
              Node.balanceLeft(BLACK, blackHeight, ln.color(RED), value, right, empty);
    ...
    private static <T> Tuple2<Node<T>, Boolean> unbalancedRight(
      ...
      if (!right.isEmpty()) {
        ...
        if (rn.color == BLACK) {
          final Node<T> newNode =
              Node.balanceRight(BLACK, blackHeight, left, value, rn.color(RED), empty);
          return Tuple.of(newNode, color == BLACK);
