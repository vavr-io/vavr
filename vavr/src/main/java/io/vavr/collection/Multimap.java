 ...
 *   <li>{@link #get(Object)}
 *   <li>{@link #getContainerType()}
 *   <li>{@link #keySet()}
 *   <li>{@link #merge(Multimap)}
 *   <li>{@link #merge(Multimap, BiFunction)}
 *   <li>{@link #put(Object, Object)}
 *   <li>{@link #put(Tuple2)}
 *   <li>{@link #values()}
 * </ul>
 *
 ...
 *
 * <ul>
 *   <li>{@link #filter(BiPredicate)}
 *   <li>{@link #filterKeys(Predicate)}
 *   <li>{@link #filterValues(Predicate)}
 *   <li>{@link #reject(BiPredicate)}
 *   <li>{@link #rejectKeys(Predicate)}
 *   <li>{@link #rejectValues(Predicate)}
 *   <li>{@link #remove(Object)}
 *   <li>{@link #remove(Object, Object)}
 *   <li>{@link #removeAll(Iterable)}
 * </ul>
 *
 * Iteration:
 *
 * <ul>
 *   <li>{@link #forEach(BiConsumer)}
 *   <li>{@link #iterator(BiFunction)}
 * </ul>
 *
 * Transformation:
 *
 * <ul>
 *   <li>{@link #bimap(Function, Function)}
 *   <li>{@link #flatMap(BiFunction)}
 *   <li>{@link #map(BiFunction)}
 *   <li>{@link #mapValues(Function)}
 *   <li>{@link #transform(Function)}
 *   <li>{@link #unzip(BiFunction)}
 *   <li>{@link #unzip3(BiFunction)}
 * </ul>
 *
 * @param <K> Key type
 * @param <V> Value type
...
public interface Multimap<K, V>
  ...
  /**
   * Defines the type of container used to store values associated with keys in a Multimap. Each
   * type determines how values are stored and how add/remove operations behave.
   */
  ...
  enum ContainerType {
    ...
    /**
     * Values are stored in a sorted {@link Set}, ensuring uniqueness and natural ordering of values
     * per key.
     */
    SORTED_SET(
        (Traversable<?> set, Object elem) -> ((Set<Object>) set).add(elem),
    ...
    /**
     * Values are stored in a {@link Seq}, allowing duplicate values per key and preserving
     * insertion order.
     */
    SEQ(
  ...
  /**
   * Narrows a widened {@code Multimap<? extends K, ? extends V>} to {@code Multimap<K, V>} by
  ...
  /**
   * Turns this {@code Multimap} into a {@link PartialFunction} which is defined at a specific
   * index, if this {@code Multimap} contains the given key. When applied to a defined key, the
   * partial function will return the {@link Traversable} of this {@code Multimap} that is
  ...
  /**
   * Maps this {@code Multimap} to a new {@code Multimap} with different component type by applying
  ...
  /**
   * Returns <code>true</code> if this multimap maps one or more keys to the specified value. This
  ...
  /**
   * Returns a new Multimap consisting of all elements with keys which do not satisfy the given
   * predicate.
   *
   * @param predicate the predicate used to test keys of elements
   * @return a new Multimap
  ...
  /**
   * Returns a new Multimap consisting of all elements with values which satisfy the given
   * predicate.
   *
  ...
  /**
   * Returns a new Multimap consisting of all elements with values which do not satisfy the given
   * predicate.
   *
   * @param predicate the predicate used to test values of elements
   * @return a new Multimap
   ...
   *
   * <p>Please use {@link #map(BiFunction)} if the result has to be of type {@code Multimap}.
   *
   * @param mapper A mapper
  ...
  /**
   * Creates a new multimap which by merging the entries of {@code this} multimap and {@code that}
   * multimap.
   *
   * <p>If collisions occur, the value of {@code this} multimap is taken.
   *
   ...
   * resolution function will always take the first argument from <code>this</code> multimap and the
   * second from <code>that</code> multimap.
   *
   ...
   * @return A merged multimap
   * @throws NullPointerException if that multimap or the given collision resolution function is
   *     null
   */
  ...
  /**
   * Associates the specified value with the specified key in this multimap. If the map previously
   ...
   * @return A new Multimap containing these elements without the entry specified by that key.
   */
   ...
   * @return A new Multimap containing these elements without the entry specified by that key and
   *     value.
   */
   ...
   * @return A new Multimap containing these elements without the entries specified by that keys.
   */
   ...
   * insertion order ({@code LinkedHashMultimap}) and sort order ({@code SortedMultimap}).
   *
  ...
  /**
   * Unzips the entries of this {@code Multimap} by mapping each key-value pair to a pair and
   * splitting them into two separate {@code Seq} collections.
   *
  ...
  /**
   * Unzips the entries of this {@code Multimap} by mapping each key-value pair to a triple and
   * splitting them into three separate {@code Seq} collections.
   *
   ...
   *
   * <p>Please use {@link #flatMap(BiFunction)} if the result should be a {@code Multimap}
   *
   * @param mapper A mapper
   * @param <U> Component type
  ...
  @Override
  <C> Map<C, ? extends Multimap<K, V>> groupBy(
   ...
   *     was associated with the key. The old map otherwise.
   */
  Multimap<K, V> replace(K key, V oldValue, V newValue);

  /**
   * Replaces each entry's values with the result of invoking the given function on that each tuple
