...
/**
 * A compositional alternative to {@code java.util.Iterator} designed for single-pass traversal of a
 * sequence.
 *
 * <p><strong>Note:</strong> Iterators maintain an internal mutable state. They are not thread-safe
 * and must not be reused or shared across operations (for example, after passing them to
 * {@linkplain io.vavr.collection.List#ofAll(Iterable)}).
 *
 * <p>The abstraction defines two fundamental operations: {@code hasNext()}, which checks whether
 * another element is available, and {@code next()}, which consumes and returns that element. If
 * {@code hasNext()} returns {@code false}, {@code next()} will throw {@code
 * NoSuchElementException}.
 *
 * <p><strong>Caution:</strong> Methods other than {@code hasNext()} and {@code next()} are
 * single-use. Once such a method has been invoked, further method calls on the same iterator are
 * not guaranteed to succeed.
 *
 * <p>In essence, an Iterator represents a traversal cursor over a collection rather than the
 * collection itself, and can therefore be consumed only once.
 *
...
public interface Iterator<T> extends java.util.Iterator<T>, Traversable<T> {
  ...
  /**
   * Creates an {@code Iterator} that traverses the elements of the provided iterables in sequence,
  ...
  /**
   * Creates an {@code Iterator} that iterates over all elements of the supplied sequence of
   * iterables, in order.
   *
  ...
  /**
   * Narrows an {@code Iterator<? extends T>} to {@code Iterator<T>} using a type-safe cast. This is
   * valid because the iterator is read-only with respect to element types.
   *
  ...
  /**
   * Creates an {@code Iterator} from the provided {@link Iterable}. This is a convenience method
   * equivalent to calling {@code Iterator.ofAll(iterable.iterator())}.
   *
  ...
  /**
   * Creates an {@code Iterator} that delegates {@code hasNext()} and {@code next()} to the given
  ...
  /**
   * Returns an {@code Iterator} over a sequence of {@code n} elements, where each element is
   * computed by the given function {@code f} applied to its index.
   *
  ...
  /**
   * Returns an {@code Iterator} over a sequence of {@code n} elements supplied by the given {@code
   * Supplier}.
   *
  ...
  /**
   * Creates an {@code Iterator} of characters starting from {@code from} (inclusive) up to {@code
   ...
   *
   * @param from the first character (inclusive)
   * @param toExclusive the end character (exclusive) - successor of the last character if {@code
   *     step > 0}, or predecessor if {@code step < 0}
   * @param step the increment between characters; must not be zero
   * @return an iterator over the specified character range, or empty if the step direction does not
  ...
  /**
   * Creates an {@code Iterator} of double values starting from {@code from} (inclusive) up to
   * {@code toExclusive} (exclusive), advancing by the specified {@code step}.
   *
   * <p>Examples:
   *
   * <pre>
   ...
   *
   * <p>This method provides precise decimal arithmetic suitable for financial calculations and
   * other scenarios where exact decimal representation is required.
   *
   * <p>Examples:
   *
   * <pre>
  ...
  /**
   * Creates an {@code Iterator} of int values starting from {@code from} (inclusive) up to {@code
   * toExclusive} (exclusive).
   *
   * <p>Examples:
   *
   ...
   *
   * @param from the first number (inclusive)
   * @param toExclusive the end number (exclusive) — last number + 1 if {@code step > 0}, or last
   *     number - 1 if {@code step < 0}
   * @param step the increment; must not be zero
   * @return an iterator over the specified range, or empty if the step direction does not match the
   *     direction from {@code from} to {@code toExclusive}, or if {@code from == toExclusive}
   * @throws IllegalArgumentException if {@code step} is zero
  ...
  /**
   * Creates an {@code Iterator} of long values starting from {@code from} (inclusive) up to {@code
   * toExclusive} (exclusive).
   *
   * <p>Examples:
   *
  ...
  /**
   * Creates an {@code Iterator} of double values starting from {@code from} (inclusive) up to
   * {@code toInclusive} (inclusive), advancing by the specified {@code step}.
   *
   * <p>Examples:
   *
   * <pre>
   ...
   * @param step the increment; must not be zero
   * @return an iterator over the specified range, or empty if the step direction does not match the
   *     direction from {@code from} to {@code toInclusive}, or if {@code from == toInclusive} it
  ...
  /**
   * Returns an infinite {@code Iterator} of int values starting from {@code value} and advancing by
   * the specified {@code step}.
   *
   * <p>The iterator wraps from {@code Integer.MAX_VALUE} to {@code Integer.MIN_VALUE} if overflow
   * occurs.
   *
   * @param value the starting int value
  ...
  /**
   * Returns an infinite {@code Iterator} of long values starting from {@code value} and advancing
   * by the specified {@code step}.
   *
   * <p>The iterator wraps from {@code Long.MAX_VALUE} to {@code Long.MIN_VALUE} if overflow occurs.
   *
   * @param value the starting long value
  ...
  /**
   * Returns an infinite {@code Iterator} that repeatedly generates values using the provided {@code
   * Supplier}.
   *
  ...
  /**
   * Creates an {@code Iterator} that repeatedly invokes the given {@code Supplier} as long as it
   * returns a {@code Some} value, terminating when it returns {@code None}.
   *
   * @param supplier the supplier providing {@code Option} values; must not be {@code null}
   * @param <T> the type of values produced
   * @return an iterator yielding the values wrapped in {@code Some}, stopping at the first {@code
   *     None}
  ...
  /**
   * Returns an infinite {@code Iterator} that generates values by repeatedly applying the given
   * function to the previous value, starting with {@code seed}.
   *
   * <p>Each call to {@code getNext()} produces the next element by applying {@code f} to the
   * previous element.
   *
  ...
  @Override
  default <R> Iterator<R> collect(
      @NonNull PartialFunction<? super T, ? extends R> partialFunction) {
    Objects.requireNonNull(partialFunction, "partialFunction is null");
    return filter(partialFunction::isDefinedAt).map(partialFunction::apply);
  }

  /**
   * Returns a new {@code Iterator} that yields the elements of this iterator followed by all
   * elements of the specified iterator.
   *
   * <p>This method appends the elements from {@code that} to the end of this iterator, creating a
  ...
  /**
   * Returns a new {@code Iterator} where the specified {@code element} is inserted between each
  ...
  @Override
  default <U, R> Iterator<R> zipWith(
  ...
  @Override
  default <U> Iterator<Tuple2<T, U>> zipAll(
  ...
  @Override
  default <U> Iterator<U> zipWithIndex(
  ...
  default <T1, T2, T3> Tuple3<Iterator<T1>, Iterator<T2>, Iterator<T3>> unzip3(
    ...
    if (!hasNext()) {
      ...
      final Stream<Tuple3<? extends T1, ? extends T2, ? extends T3>> source =
          Stream.ofAll(this.map(unzipper));
      return Tuple.of(
          source.map(t -> (T1) t._1).iterator(),
          source.map(t -> (T2) t._2).iterator(),
          source.map(t -> (T3) t._3).iterator());
    }
  }
   ...
   *
   * <p>The function takes the current seed and returns {@code None} to signal the end of iteration,
  ...
  /**
   * Creates an {@code Iterator} by repeatedly applying a function to a seed value, generating
   * elements in a left-to-right order.
   *
   * <p>The function receives the current seed and returns {@code None} to signal the end of
   * iteration, or {@code Some<Tuple2>} containing the next seed and the element to include in the
   * iterator.
   *
   * <p>Example:
   *
  ...
  /**
   * Creates an {@code Iterator} by repeatedly applying a function to a seed value, generating
   * elements in a right-to-left order.
   *
   * <p>The function receives the current seed and returns {@code None} to signal the end of
   * iteration, or {@code Some<Tuple2>} containing the element to yield and the next seed for
   * subsequent calls.
   *
   * <p>Example:
   *
   ...
   * keeping the last occurrence of each duplicate element, as determined by the given {@code
   * comparator}.
   *
   * <p>When multiple elements are considered equal according to the comparator, only the last
   * occurrence in the sequence is retained.
   *
   * <p>Examples:
   *
   * <pre>
  ...
  default Iterator<T> distinctByKeepLast(Comparator<? super T> comparator) {
    ...
    if (!hasNext()) {
      ...
      return Collections.reverseIterator(
          new DistinctIterator<>(
   ...
   *
   * <p>When multiple elements have the same extracted key, only the last occurrence in the sequence
   * is retained.
   *
   * <p>Examples:
   *
  ...
  default <U> Iterator<T> distinctByKeepLast(Function<? super T, ? extends U> keyExtractor) {
    ...
    if (!hasNext()) {
      ...
      return Collections.reverseIterator(
          new DistinctIterator<>(
              Collections.reverseIterator(this), io.vavr.collection.HashSet.empty(), keyExtractor));
    }
  }
   ...
   *
   * @param n A number
   * @return The empty iterator, if {@code n <= 0} or this is empty, otherwise a new iterator
  ...
  /**
   * FlatMaps the elements of this Iterator to Iterables, which are iterated in the order of
   * occurrence.
   *
  ...
  @Override
  default Traversable<T> scan(
      T zero, @NonNull BiFunction<? super T, ? super T, ? extends T> operation) {
    return scanLeft(zero, operation);
  }

  @Override
  default <U> Iterator<U> scanLeft(
  ...
  // not lazy!
  @Override
  default <U> Iterator<U> scanRight(
...
interface IteratorModule {
  ...
  /**
   * Creates two new iterators that both iterates over the same elements as this iterator and in the
   * same order. The duplicate iterators are considered equal if they are positioned at the same
   * element.
   *
   * <p>Given that most methods on iterators will make the original iterator unfit for further use,
   * this method provides a reliable way of calling multiple such methods on an iterator.
   *
  ...
  final class DistinctIterator<T, U> extends AbstractIterator<T> {
    ...
    DistinctIterator(
        Iterator<? extends T> that, Set<U> set, Function<? super T, ? extends U> keyExtractor) {
      this.that = that;
      this.known = set;
    ...
    GroupedIterator(Iterator<T> that, int size, int step) {
      if (size < 1 || step < 1) {
        throw new IllegalArgumentException(
            "size (" + size + ") and step (" + step + ") must both be positive");
      }
      this.that = that;
      this.size = size;
      this.step = step;
  ...
  final class BigDecimalHelper {
    ...
    private static final Lazy<BigDecimal> INFINITY_DISTANCE =
        Lazy.of(
            () -> {
              final BigDecimal two = BigDecimal.valueOf(2);
              final BigDecimal supremum =
                  BigDecimal.valueOf(Math.nextDown(Double.POSITIVE_INFINITY));
