 ...
 *
 * <p>This helper class provides methods that return views on Java collections. The view creation
 * and back conversion take O(1).
 *
...
class JavaConverters {
   ...
   */
  private abstract static class HasDelegate<C extends Traversable<?>> implements Serializable {
  ...
  static class ListView<T, C extends Seq<T>> extends HasDelegate<C> implements java.util.List<T> {
    ...
    public boolean containsAll(@NonNull Collection<?> collection) {
      Objects.requireNonNull(collection, "collection is null");
      @SuppressWarnings("unchecked")
      final Collection<T> that = (Collection<T>) collection;
    ...
    public T set(int index, T element) {
      return setDelegateAndGetPreviousElement(
          index, () -> (C) getDelegate().update(index, element));
    }
    ...
    public boolean equals(Object o) {
      return o == this
          || o instanceof java.util.List
              && Collections.areEqual(getDelegate(), (java.util.List<?>) o);
    }
    ...
    private static class Iterator<T, C extends Seq<T>> implements java.util.Iterator<T> {
      ...
      public void forEachRemaining(Consumer<? super T> consumer) {
        ...
        int index = nextIndex;
        // DEV-NOTE: intentionally not using hasNext() and next() in order not to modify internal
        // state
    ...
    private static class ListIterator<T, C extends Seq<T>> extends ListView.Iterator<T, C>
      ...
      public T previous() {
        ...
        try {
          ...
          // DEV-NOTE: intentionally updating indices _after_ reading the element. This makes a
          // difference in case of a concurrent modification.
      ...
      public void add(T element) {
        ...
        try {
          ...
          // DEV-NOTE: intentionally increasing nextIndex _after_ adding the element. This makes a
          // difference in case of a concurrent modification.
          nextIndex = index + 1;
          lastIndex = -1;
          expectedSize = list.size();
