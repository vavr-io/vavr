...
/**
 * Represents a recursive, multi-valued data structure whose elements can be traversed in order. The
 * iteration order is determined by {@link Iterable#iterator()} and may vary across calls.
 *
 * <p>This interface provides operations for:
 *
 * <ul>
 *   <li><b>Basic access:</b> querying elements, length, head/tail, and emptiness.
 *   <li><b>Iteration:</b> indexed traversal, sliding windows, and grouping.
 *   <li><b>Numeric computations:</b> sum, product, min/max, and averages.
 *   <li><b>Reduction and folding:</b> folding, reducing, and string representation.
 *   <li><b>Selection and slicing:</b> take/drop, filtering, partitioning, and sub-sequencing.
 *   <li><b>Testing:</b> uniqueness, order, distinctness, and sequence properties.
 *   <li><b>Transformation:</b> mapping, flat-mapping, scanning, zipping, and deduplication.
 * </ul>
 *
...
public interface Traversable<T> extends Foldable<T>, Value<T> {
   ...
   *
   * <p>This is safe because immutable or read-only collections are covariant in their element type.
   *
   ...
   *
   * <p>Returns {@code None} if any key occurs more than once; otherwise, returns a {@code Map}
   ...
   * @return an {@code Option} containing the {@code Map} of elements by key, or {@code None} if
   *     keys are not unique
   ...
   *
   * <p>If the elements are not numeric, an {@link UnsupportedOperationException} is thrown.
   *
   * <p>Examples:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * <p>Unlike Java's {@link DoubleStream#average()} which uses the Kahan summation algorithm, Vavr
   * uses Neumaier's modification of Kahan's algorithm for improved numerical accuracy.
   *
  ...
  /**
   * Applies a {@link PartialFunction} to all elements that are defined for it and collects the
   * results.
   *
   * <p>For each element in iteration order, the function is first tested:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * <p><strong>Note:</strong> If this {@code Traversable} is ordered (i.e., extends {@link
   * Ordered}), the caller must ensure that the resulting elements are comparable (i.e., implement
   * {@link Comparable}).
   *
   ...
   *
   * <p>Equivalent to testing each element individually:
   *
   * <pre>{@code
  ...
  /**
   * Returns a new {@code Traversable} containing the elements of this instance with all duplicates
   * removed. Element equality is determined using {@code equals}.
   *
  ...
  /**
   * Returns a new {@code Traversable} containing the elements of this instance without duplicates,
   * as determined by the given {@code comparator}.
   *
  ...
  /**
   * Returns a new {@code Traversable} containing the elements of this instance without duplicates,
   * based on keys extracted from elements using {@code keyExtractor}.
   *
   * <p>The first occurrence of each key is retained in the resulting sequence.
   *
  ...
  /**
   * Returns a new {@code Traversable} without the first {@code n} elements, or an empty instance if
   * this contains fewer than {@code n} elements.
   *
  ...
  /**
   * Returns a new {@code Traversable} without the last {@code n} elements, or an empty instance if
   * this contains fewer than {@code n} elements.
   *
  ...
  /**
   * Returns a new {@code Traversable} starting from the first element that satisfies the given
   * {@code predicate}, dropping all preceding elements.
   *
  ...
  /**
   * Returns a new {@code Traversable} starting from the first element that does not satisfy the
   * given {@code predicate}, dropping all preceding elements.
   *
   * <p>This is equivalent to {@code dropUntil(predicate.negate())}, which is useful for method
   * references that cannot be negated directly.
   *
   * @param predicate a condition tested on each element
   ...
   *
   * <p>In Vavr, there are four basic collection types:
   *
   * <ul>
   *   <li>{@code Seq} – sequential elements
   *   <li>{@code Set} – distinct elements
   *   <li>{@code Map} – key-value pairs
   *   <li>{@code Multimap} – keys mapped to multiple values
   * </ul>
   *
   * Two collections are considered equal if and only if:
   *
   * <ul>
   *   <li>They are of the same collection type (Seq, Set, Map, Multimap)
   *   <li>They contain the same elements
   *   <li>For {@code Seq}, the element order is the same
   * </ul>
   *
   * <p>For {@code Map} and {@code Multimap}, two entries {@code (key1, value1)} and {@code (key2,
   * value2)} are equal if both their keys and values are equal.
   *
   * <p><strong>Additional notes:</strong>
   *
   * <ul>
   *   <li>No collection equals {@code null} (e.g., {@code Queue(1) != null})
   *   <li>Null elements are allowed and treated as expected (e.g., {@code List(null, 1) ==
   *       Stream(null, 1)}, {@code HashMap((null,1)) == LinkedHashMap((null,1))})
   *   <li>Element order matters only for {@code Seq}
   *   <li>Other collection classes are equal if their types and elements (in iteration order) are
   *       equal
   *   <li>Iterators are compared by reference only
   * </ul>
   *
   * @param obj the object to compare with, may be {@code null}
   * @return {@code true} if the collections are equal according to the rules above, {@code false}
   *     otherwise
   */
  boolean equals(Object obj);

  /**
   * Checks whether there is exactly one element in this traversable for which the given predicate
   * holds.
   *
   ...
   *
   * @param predicate the condition to test elements
   * @return {@code Some(element)} if a matching element is found, otherwise {@code None}; the
   ...
   *
   * <p>Equivalent to {@code reverse().find(predicate)} but potentially more efficient.
   *
   * @param predicate the condition to test elements
   * @return {@code Some(element)} if a matching element is found, otherwise {@code None}; the
   *     element may be {@code null}
  ...
  /**
   * Transforms each element of this Traversable into an {@code Iterable} of elements and flattens
   * the resulting iterables into a single Traversable.
   *
   * @param mapper a function mapping elements to iterables
   * @param <U> the type of elements in the resulting Traversable
   * @return a new Traversable containing all elements produced by applying {@code mapper} and
   *     flattening
   ...
   *
   * <p>This method is more efficient than {@code iterator().zipWithIndex().forEach()} because it
   * avoids creating intermediate {@code Tuple2} objects and boxing integers.
   *
   * <p>Note that the iteration order may vary between calls depending on the underlying Traversable
   * implementation. Also, if this Traversable is lazily evaluated (e.g., a {@link Stream}), the
   * method may loop indefinitely.
   *
   ...
   */
  <C> Map<C, ? extends Traversable<T>> groupBy(
      @NonNull Function<? super T, ? extends C> classifier);

  /**
   * Splits this {@code Traversable} into consecutive blocks of the given size.
   *
   * <p>Let {@code length} be the number of elements in this {@code Traversable}:
   *
   * <ul>
   *   <li>If empty, the resulting {@code Iterator} is empty.
   *   <li>If {@code size <= length}, the resulting {@code Iterator} contains {@code length / size}
   *       blocks of size {@code size} and possibly a final smaller block of size {@code length %
   *       size}.
   *   <li>If {@code size > length}, the resulting {@code Iterator} contains a single block of size
   *       {@code length}.
   * </ul>
   *
   * <p>Examples:
   ...
   * }</pre>
   *
   * <p>Note: {@code grouped(size)} is equivalent to {@code sliding(size, size)}.
   *
   ...
   *
   * <p>This should typically be implemented by concrete classes, not interfaces.
   *
   ...
   *
   * <p>Vavr distinguishes between collections with predictable iteration order (like {@code Seq})
   * and collections with arbitrary iteration order (like {@code Set}, {@code Map}, and {@code
   * Multimap}). In all cases, the hash of an empty collection is defined as {@code 1}.
   *
   * <p>For collections with predictable iteration order, the hash is computed as:
   *
   * <pre>{@code
   * int hash = 1;
   * for (T t : this) {
   ...
   * }</pre>
   *
   * <p>For collections with arbitrary iteration order, the hash is computed to be independent of
   * element order:
   *
   * <pre>{@code
   * int hash = 1;
   ...
   * }</pre>
   *
   * <p>Note that these algorithms may change in future Vavr versions. Hash codes are generally
   * <em>not</em> cached, unlike size/length, because caching would increase memory usage due to
   * persistent tree-based structures. Computing the hash code is linear in time, O(n). For
   * frequently reused collections (e.g., as {@code HashMap} keys), caching can be done externally
   * using a wrapper, for example:
   *
   * <pre>{@code
   ...
   *
   * <p>This is the dual of {@link #tail()}.
   *
   ...
   *
   * <p>This is the dual of {@link #tailOption()}.
   *
   ...
   *
   * @return {@code true} if this Traversable may contain only distinct elements, {@code false}
   *     otherwise
   */
   ...
   *
   * <p>Implementations should provide the correct behavior; this is not meant for interfaces alone.
   *
   * @return {@code true} if this Traversable is guaranteed to be repeatably traversable, {@code
   *     false} otherwise
   */
  boolean isTraversableAgain();

  /**
   * Returns an iterator over the elements of this Traversable, implemented via {@link #head()} and
   * {@link #tail()}. Subclasses may override for a more efficient implementation.
   *
   ...
   *
   * <p>Note that the underlying order of sorted collections is not considered—only the natural
   ...
   *
   * <p>If the Traversable is empty, {@code None} is returned.
   *
  ...
  /**
   * Returns the element of this Traversable whose mapped value, according to the given function, is
   * maximal.
   *
   * <p>The mapping function {@code f} transforms elements of type {@code T} to a comparable type
   * {@code U}, and the element with the largest {@code U} value is returned.
   *
   ...
   * @return {@code Some(element)} whose mapped value is maximal, or {@code None} if this
   *     Traversable is empty
   * @throws NullPointerException if {@code f} is null
   */
  default <U extends Comparable<? super U>> Option<T> maxBy(
   ...
   *
   * <p>The underlying order of sorted collections is not considered. For numeric types {@link
   * Double} and {@link Float}, if any element is {@code NaN}, the result is {@code NaN} instead of
   ...
   * @throws ClassCastException if the elements do not have a natural order, i.e., do not implement
   *     {@link Comparable}
   */
  @SuppressWarnings("unchecked")
  ...
  /**
   * Returns the element of this Traversable whose mapped value is minimal according to natural
   * order.
   *
   * @param f a function mapping elements to a comparable value
   * @param <U> the type of the comparable value
   * @return the element of type T whose mapped value is minimal, wrapped in {@code Some}, or {@code
   *     None} if empty
   * @throws NullPointerException if {@code f} is null
   */
  default <U extends Comparable<? super U>> Option<T> minBy(
      @NonNull Function<? super T, ? extends U> f) {
    Objects.requireNonNull(f, "f is null");
    if (isEmpty()) {
      return Option.none();
   ...
   *
   * <p>Equivalent to {@code mkCharSeq("", "", "")}.
   *
  ...
  /**
   * Concatenates the string representations of all elements in this Traversable, separated by a
   * delimiter.
   *
   * <p>Equivalent to {@code mkCharSeq("", delimiter, "")}.
   *
  ...
  /**
   * Concatenates the string representations of all elements in this Traversable with a prefix,
   * delimiter, and suffix.
   *
   * <p>Example: {@code List.of("a", "b", "c").mkCharSeq("Chars(", ", ", ")") = CharSeq.of("Chars(a,
   * b, c)")}
   *
   ...
   * delimiter.
   *
   * <p>Equivalent to {@code mkString("", delimiter, "")}.
   *
   * @param delimiter a string placed between elements
   ...
   * delimiter, and suffix.
   *
   * <p>Example: {@code List.of("a", "b", "c").mkString("Chars(", ", ", ")") = "Chars(a, b, c)"}
   *
   * @param prefix a string prepended to the result
   * @param delimiter a string placed between elements
   * @param suffix a string appended to the result
  ...
  /**
   * Returns this {@code Traversable} if it is non-empty; otherwise, returns the result of
   ...
   *
   * <p>The first partition contains all elements that satisfy the predicate, and the second
   * contains all elements that do not. The original iteration order is preserved.
   *
   ...
   *
   * <p>Supported element types are {@code Byte}, {@code Double}, {@code Float}, {@code Integer},
   * {@code Long}, {@code Short}, {@code BigInteger}, and {@code BigDecimal}.
   *
   * <p>Examples:
   *
   * <pre>{@code
   ...
   * }</pre>
   *
   * <p>For type-safe multiplication of elements, consider using {@link #fold(Object, BiFunction)}.
   *
      ...
      try {
        ...
        if (o instanceof Integer || o instanceof Long || o instanceof Byte || o instanceof Short) {
          return ((Iterator<Number>) iter)
              .foldLeft(
                  ((Number) o).longValue(), (product, number) -> product * number.longValue());
   ...
   * @return a new Traversable with the first occurrence of {@code currentElement} replaced by
   *     {@code newElement}
   */
   ...
   * @param currentElement the element to be replaced
   * @param newElement the replacement element
   * @return a new Traversable with all occurrences of {@code currentElement} replaced by {@code
   *     newElement}
   */
  Traversable<T> replaceAll(T currentElement, T newElement);

  /**
   * Retains only the elements from this Traversable that are contained in the given {@code
   * elements}.
   *
   * @param elements the elements to keep
   * @return a new Traversable containing only the elements present in {@code elements}, in their
   *     original order
   * @throws NullPointerException if {@code elements} is null
   */
   ...
   *
   * <p>The neutral element {@code zero} may be applied more than once.
   *
  ...
  /**
   * Produces a collection containing cumulative results of applying the operator from left to
   * right.
   *
   * <p>Will not terminate for infinite collections. The results may vary across runs unless the
  ...
  /**
   * Produces a collection containing cumulative results of applying the operator from right to
   * left.
   *
   * <p>The head of the resulting collection is the last cumulative result. Will not terminate for
   * infinite collections. Results may vary across runs unless the collection is ordered.
   *
   * @param <U> the type of the resulting elements
   * @param zero the initial value
   ...
   *
   * @return {@code Some(element)} if the Traversable contains exactly one element, or {@code None}
   *     otherwise.
   */
  ...
  /**
   * Partitions this {@code Traversable} into consecutive non-overlapping windows according to a
   * classification function.
   *
   * <p>Each window contains elements with the same class, as determined by {@code classifier}. Two
   * consecutive elements belong to the same window only if {@code classifier} returns equal values
   ...
   *
   * <p>This is equivalent to calling {@link #sliding(int, int)} with a step size of 1.
   *
  ...
  /**
   * Slides a window of a specific {@code size} with a given {@code step} over this {@code
   * Traversable}.
   *
   * <p>Examples:
   *
  ...
  /**
   * Splits this {@code Traversable} into a prefix and remainder according to the given {@code
   * predicate}.
   *
   * <p>The first element of the returned {@code Tuple} is the longest prefix of elements satisfying
   * {@code predicate}, and the second element is the remaining elements.
   *
   ...
   * }</pre>
   *
   * <p>See also {@link #fold(Object, BiFunction)} for type-safe summation of elements.
   *
    ...
    if (isEmpty()) {
      ...
      try {
        ...
        if (o instanceof Integer || o instanceof Long || o instanceof Byte || o instanceof Short) {
          ...
          return TraversableModule.neumaierSum(
              Iterator.of(o).concat(iter), t -> ((Number) t).doubleValue())[0];
        }
      } catch (ClassCastException x) {
        throw new UnsupportedOperationException("Elements are not numeric", x);
      }
  ...
  /**
   * Returns the first {@code n} elements of this {@code Traversable}, or all elements if {@code n}
   * exceeds the length.
   *
   * <p>Equivalent to {@code sublist(0, max(0, min(length(), n)))}, but safe for {@code n < 0} or
   * {@code n > length()}.
   *
   * <p>If {@code n < 0}, an empty instance is returned. If {@code n > length()}, the full instance
   * is returned.
   *
  ...
  /**
   * Returns the last {@code n} elements of this {@code Traversable}, or all elements if {@code n}
   * exceeds the length.
   *
   * <p>Equivalent to {@code sublist(max(0, length() - n), length())}, but safe for {@code n < 0} or
   * {@code n > length()}.
   *
   * <p>If {@code n < 0}, an empty instance is returned. If {@code n > length()}, the full instance
   * is returned.
   ...
   *
   * <p>Equivalent to {@code takeWhile(predicate.negate())}, but useful when using method references
   * that cannot be negated directly.
   *
   * @param predicate a condition tested sequentially on the elements
   * @return a new {@code Traversable} containing all elements before the first one that satisfies
   *     the predicate
   * @throws NullPointerException if {@code predicate} is null
   */
   ...
   *
   * @param predicate a condition tested sequentially on the elements
   * @return a new {@code Traversable} containing all elements up to (but not including) the first
  ...
  /**
   * Unzips the elements of this {@code Traversable} by mapping each element to a pair and splitting
   * them into two separate {@code Traversable} collections.
   *
  ...
  /**
   * Unzips the elements of this {@code Traversable} by mapping each element to a triple and
   * splitting them into three separate {@code Traversable} collections.
   *
   ...
   * @return a {@code Tuple3} containing three {@code Traversable} collections with the split
   *     elements
   * @throws NullPointerException if {@code unzipper} is null
   */
  <T1, T2, T3>
      Tuple3<? extends Traversable<T1>, ? extends Traversable<T2>, ? extends Traversable<T3>>
          unzip3(
              @NonNull Function<? super T, Tuple3<? extends T1, ? extends T2, ? extends T3>>
                  unzipper);

  /**
   * Returns a {@code Traversable} formed by pairing elements of this {@code Traversable} with
   * elements of another {@code Iterable}. Pairing stops when either collection runs out of
   * elements; any remaining elements in the longer collection are ignored.
   *
   * <p>The length of the resulting {@code Traversable} is the minimum of the lengths of this {@code
   * Traversable} and {@code that}.
   *
  ...
  /**
   * Returns a {@code Traversable} formed by pairing elements of this {@code Traversable} with
   * elements of another {@code Iterable}, filling in placeholder elements when one collection is
   * shorter than the other.
   *
   * <p>The length of the resulting {@code Traversable} is the maximum of the lengths of this {@code
   * Traversable} and {@code that}.
   *
   * <p>If this {@code Traversable} is shorter than {@code that}, {@code thisElem} is used as a
   * filler. Conversely, if {@code that} is shorter, {@code thatElem} is used.
   *
   * @param <U> the type of elements in the second half of each pair
   * @param that an {@code Iterable} providing the second element of each pair
   * @param thisElem the element used to fill missing values if this {@code Traversable} is shorter
   *     than {@code that}
   * @param thatElem the element used to fill missing values if {@code that} is shorter than this
   *     {@code Traversable}
  ...
  /**
   * Returns a {@code Traversable} by combining elements of this {@code Traversable} with elements
   * of another {@code Iterable} using a mapping function. Pairing stops when either collection runs
   * out of elements.
   *
   * <p>The length of the resulting {@code Traversable} is the minimum of the lengths of this {@code
   * Traversable} and {@code that}.
  ...
  /**
   * Zips this {@code Traversable} with its indices and maps the resulting pairs using the provided
   * mapper.
   *
...
interface TraversableModule {
   ...
   *
   * <p>See <a href="https://en.wikipedia.org/wiki/Kahan_summation_algorithm">Kahan summation
   * algorithm</a>.
   *
   * @param <T> element type
   * @param ts the elements
   * @param toDouble function which maps elements to {@code double} values
   * @return A pair {@code [sum, size]}, where {@code sum} is the compensated sum and {@code size}
