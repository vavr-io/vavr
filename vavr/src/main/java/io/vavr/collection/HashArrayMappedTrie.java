...
/**
 * An immutable <a href="https://en.wikipedia.org/wiki/Hash_array_mapped_trie">Hash array mapped
 * trie (HAMT)</a>.
 *
...
interface HashArrayMappedTrieModule {
  ...
  class LeafNodeIterator<K, V> extends AbstractIterator<LeafNode<K, V>> {
    ...
    private static final int MAX_LEVELS = Integer.SIZE / AbstractNode.SIZE + 2;
    ...
    private static <K, V> int downstairs(
        Object[] nodes, int[] indexes, AbstractNode<K, V> root, int level) {
      while (true) {
        nodes[level] = root;
        indexes[level] = 0;
    ...
    private static <K, V> AbstractNode<K, V> getChild(AbstractNode<K, V> node, int index) {
      if (node instanceof IndexedNode) {
        ...
        return index < AbstractNode.BUCKET_SIZE
            ? (AbstractNode<K, V>) arrayNode.subNodes[index]
            : null;
      }
      return null;
    }
  ...
  abstract class LeafNode<K, V> extends AbstractNode<K, V> {
    ...
    static <K, V> AbstractNode<K, V> mergeLeaves(
        int shift, LeafNode<K, V> leaf1, LeafSingleton<K, V> leaf2) {
    ...
    AbstractNode<K, V> modify(int shift, int keyHash, K key, V value, Action action) {
      if (keyHash == hash && Objects.equals(key, this.key)) {
        ...
        return (action == REMOVE)
            ? this
            : mergeLeaves(shift, this, new LeafSingleton<>(keyHash, key, value));
      }
    }
  ...
  final class LeafList<K, V> extends LeafNode<K, V> implements Serializable {
    ...
    private static <K, V> AbstractNode<K, V> mergeNodes(
        LeafNode<K, V> leaf1, LeafNode<K, V> leaf2) {
      if (leaf2 == null) {
        return leaf1;
      }
    ...
    AbstractNode<K, V> modify(int shift, int keyHash, K key, V value, Action action) {
      ...
      if (newBitmap == 0) {
        ...
        if (!exists) {
          ...
          return new IndexedNode<>(
              newBitmap, size - atIndx.size() + child.size(), update(subNodes, index, child));
        }
      }
    }

    private ArrayNode<K, V> expand(
        int frag, AbstractNode<K, V> child, int mask, Object[] subNodes) {
      int bit = mask;
      int count = 0;
      int ptr = 0;
      ...
      if (child.isEmpty() && !newChild.isEmpty()) {
        ...
        if (count - 1 <= MIN_ARRAY_NODE) {
          ...
          return new ArrayNode<>(
              count - 1, size - child.size(), update(subNodes, frag, EmptyNode.instance()));
        }
      } else {
        return new ArrayNode<>(
            count, size - child.size() + newChild.size(), update(subNodes, frag, newChild));
      }
    }
