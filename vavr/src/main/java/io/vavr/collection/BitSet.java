...
public interface BitSet<T> extends SortedSet<T> {
  ...
  class Builder<T> {
    ...
    static final Builder<Integer> DEFAULT = new Builder<>(i -> i, i -> i);
     ...
     * java.util.stream.Stream#collect(java.util.stream.Collector)} to obtain an {@link ArrayList}.
     *
  ...
  /**
   * Returns a {@link java.util.stream.Collector} which may be used in conjunction with {@link
   * java.util.stream.Stream#collect(java.util.stream.Collector)} to obtain a {@link BitSet}.
   *
  ...
  /**
   * Creates a BitSet of int numbers starting from {@code from}, extending to {@code toExclusive -
   * 1}.
   *
  ...
  /**
   * Returns a BitSet containing {@code n} values of a given Function {@code f} over a range of
   ...
   * @return A BitSet of size {@code n}, where each element contains the result supplied by {@code
   *     s}.
  ...
  /**
   * Creates a BitSet of char numbers starting from {@code from}, extending to {@code toExclusive -
   * 1}.
   *
   * @param from the first number
   * @param toExclusive the last number + 1
  ...
  /**
   * Creates a BitSet of long numbers starting from {@code from}, extending to {@code toExclusive -
   * 1}.
   *
   * @param from the first number
   * @param toExclusive the last number + 1
  ...
  default <U> TreeSet<Tuple2<T, U>> zip(@NonNull Iterable<? extends U> that) {
    ...
    final Comparator<Tuple2<T, U>> tuple2Comparator =
        Tuple2.comparator(comparator(), Comparators.naturalComparator());
    return TreeSet.ofAll(tuple2Comparator, iterator().zip(that));
  }

  @Override
  default <U, R> TreeSet<R> zipWith(
  ...
  @Override
  default <U> TreeSet<Tuple2<T, U>> zipAll(
  ...
  @Override
  default <U> TreeSet<U> zipWithIndex(
...
interface BitSetModule {
  ...
  abstract class AbstractBitSet<T> implements BitSet<T>, Serializable {
    ...
    /**
     * Returns this {@code BitSet} if it is nonempty, otherwise {@code BitSet} created from
     * iterable, using existing bitset properties.
     *
     * @param other An alternative {@code Traversable}
     * @return this {@code BitSet} if it is nonempty, otherwise {@code BitSet} created from
     *     iterable, using existing bitset properties.
     */
    @Override
    ...
    /**
     * Returns this {@code BitSet} if it is nonempty, otherwise {@code BitSet} created from result
     * of evaluating supplier, using existing bitset properties.
     *
     * @param supplier An alternative {@code Traversable}
     * @return this {@code BitSet} if it is nonempty, otherwise {@code BitSet} created from result
     *     of evaluating supplier, using existing bitset properties.
     */
    @Override
    ...
    public BitSet<T> init() {
      if (isEmpty()) {
        ...
        final int element =
            BITS_PER_WORD * (getWordsNum() - 1)
                + BITS_PER_WORD
                - Long.numberOfLeadingZeros(last)
                - 1;
