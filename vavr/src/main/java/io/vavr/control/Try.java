...
/**
 * A control structure that allows writing safe code without explicitly managing try-catch blocks
 * for exceptions.
 *
 * <p>The following exceptions are considered fatal or non-recoverable:
 *
 * <ul>
 *   <li>{@linkplain InterruptedException}
 *   <li>{@linkplain LinkageError}
 *   <li>{@linkplain ThreadDeath}
 *   <li>{@linkplain VirtualMachineError}, including {@linkplain OutOfMemoryError} and {@linkplain
 *       StackOverflowError}
 * </ul>
 *
 * <p><strong>Note:</strong> Methods such as {@code get()} may re-throw exceptions without declaring
 * them. When used within a {@link java.lang.reflect.InvocationHandler} of a dynamic proxy, such
 * exceptions will be wrapped in {@link java.lang.reflect.UndeclaredThrowableException}. See <a
 * href="https://docs.oracle.com/javase/8/docs/technotes/guides/reflection/proxy.html">Dynamic Proxy
 * Classes</a> for more details.
 *
...
public interface Try<T> extends Value<T>, Serializable {
   ...
   *
   * <p>If the supplier executes without throwing an exception, a {@link Success} containing the
   * result is returned. If an exception occurs during execution, a {@link Failure} wrapping the
   ...
   * @return a {@link Success} with the supplier's result, or a {@link Failure} if an exception is
   *     thrown
   ...
   *
   * <p>If the callable executes without throwing an exception, a {@link Success} containing the
   * result is returned. If an exception occurs during execution, a {@link Failure} wrapping the
   * thrown exception is returned.
   *
   * @param callable the callable to execute
   * @param <T> the type of the value returned by the callable
   * @return a {@link Success} with the callable's result, or a {@link Failure} if an exception is
   *     thrown
   ...
   *
   * <p>If the runnable executes without throwing an exception, a {@link Success} containing {@code
   * null} (representing the absence of a value) is returned. If an exception occurs during
   * execution, a {@link Failure} wrapping the thrown exception is returned.
   *
   * @param runnable the checked runnable to execute
   * @return a {@link Success} with {@code null} if the runnable completes successfully, or a {@link
  ...
  /**
   * Transforms an {@link Iterable} of {@link Try} instances into a single {@link Try} containing a
   * {@link Seq} of all successful results.
   *
   * <p>If any element in the input iterable is a {@link Try.Failure}, the resulting {@link Try}
   * will also be a {@link Try.Failure}, containing the first encountered failure's cause.
   *
   * @param values an {@link Iterable} of {@code Try} instances
   * @param <T> the type of values contained in the {@code Try}s
   * @return a {@link Try} containing a {@link Seq} of all successful results, or a {@link
   *     Try.Failure} if any input is a failure
  ...
  /**
   * Transforms an {@link Iterable} of values into a single {@link Try} containing a {@link Seq} of
   * mapped results.
   *
   * <p>Each value in the input iterable is mapped using the provided {@code mapper} function, which
   * produces a {@link Try}. If all mappings succeed, a {@link Success} containing a {@link Seq} of
   * results is returned. If any mapping results in a {@link Try.Failure}, the first encountered
   * failure is returned.
   *
   ...
   * @return a {@link Try} containing a {@link Seq} of all mapped results, or a {@link Try.Failure}
   ...
   *
   * <p>This is a convenience method equivalent to {@code new Success<>(value)}.
   *
   ...
   *
   * <p>This is a convenience method equivalent to {@code new Failure<>(exception)}.
   *
   ...
   *
   * <p>This is safe because {@code Try} is immutable and its contents are read-only, allowing
   * covariance.
   *
  ...
  /**
   * Performs the given {@link Consumer} on the value of this {@code Try} if it is a {@link
   * Success}.
   *
   * <p>This is a shortcut for {@code andThenTry(consumer::accept)}. If this {@code Try} is a {@link
   * Failure}, it is returned unchanged. If the consumer throws an exception, a {@link Failure} is
   * returned.
   *
   * @param consumer the consumer to execute on the value
   * @return this {@code Try} if it is a {@link Failure} or the consumer succeeds, otherwise a
   *     {@link Failure} of the consumer
  ...
  /**
   * Passes the result of this {@code Try} to the given {@link CheckedConsumer} if this is a {@link
   * Success}.
   *
   * <p>This allows chaining of operations that may throw checked exceptions. If this {@code Try} is
   * a {@link Failure}, it is returned unchanged. If the consumer throws an exception, a {@link
   * Failure} containing that exception is returned.
   *
   * <p>Example usage:
   *
   * <pre>{@code
   ...
   *
   * <p>This is a shortcut for {@code andThenTry(runnable::run)}. If this {@code Try} is a {@link
   * Failure}, it is returned unchanged. If the runnable throws an exception, a {@link Failure}
   * containing that exception is returned.
   *
   * @param runnable the runnable to execute
   * @return this {@code Try} if it is a {@link Failure} or the runnable succeeds, otherwise a
   *     {@link Failure} of the runnable
   * @throws NullPointerException if {@code runnable} is {@code null}
  ...
  /**
   * Executes the given {@link CheckedRunnable} if this {@code Try} is a {@link Success}; otherwise,
   * returns this {@code Failure}.
   *
   * <p>This allows chaining of runnables that may throw checked exceptions. If the runnable throws
   * an exception, a {@link Failure} containing that exception is returned.
   *
   * <p>Example usage with method references:
   *
   * <pre>{@code
   * Try.run(A::methodRef)
   *    .andThen(B::methodRef)
  ...
  /**
   * Transforms the value of this {@code Try} using the given {@link PartialFunction} if it is
   * defined at the value.
   *
   * <p>The {@code partialFunction} is first tested with {@link
   * PartialFunction#isDefinedAt(Object)}. If it returns {@code true}, the value is mapped using
   * {@link PartialFunction#apply(Object)} and wrapped in a new {@code Try}. If the function is not
   * defined at the value or this {@code Try} is a {@link Failure}, the result is a {@link Failure}.
   *
   * <p>Example:
   *
   * <pre>{@code
   ...
   *
   * <p>If this {@code Try} is a {@link Success}, a {@code Failure} containing a {@link
   * NoSuchElementException} is returned.
   *
   * @return a {@code Try<Throwable>} representing the throwable of this {@link Failure}, or a
   *     {@link Failure} if this is a {@link Success}
   */
  ...
  /**
   * Returns a {@code Try} if the given {@link Predicate} evaluates to {@code true}, otherwise
   * returns a {@link Failure} created by the given {@link Supplier} of {@link Throwable}.
   *
   * <p>This is a shortcut for {@link #filterTry(CheckedPredicate, Supplier)}.
   *
   ...
   * @return this {@code Try} if the predicate passes, otherwise a {@link Failure} from the
   *     throwable supplier
   * @throws NullPointerException if {@code predicate} or {@code throwableSupplier} is {@code null}
   */
  default Try<T> filter(
      @NonNull Predicate<? super T> predicate, Supplier<? extends Throwable> throwableSupplier) {
  ...
  /**
   * Returns a {@code Try} if the given {@link Predicate} evaluates to {@code true}, otherwise
   * returns a {@link Failure} created by applying the given function to the value.
   *
   * <p>This is a shortcut for {@link #filterTry(CheckedPredicate, CheckedFunction1)}.
   *
   * @param predicate the predicate to test the value
   * @param errorProvider a function providing a throwable if the predicate fails
   * @return this {@code Try} if the predicate passes, otherwise a {@link Failure} from the error
   *     provider
  ...
  /**
   * Returns a {@code Try} if the given {@link Predicate} evaluates to {@code true}, otherwise
   * returns a {@link Failure}.
   *
   * <p>This is a shortcut for {@link #filterTry(CheckedPredicate)}.
   *
   * @param predicate the predicate to test the value
  ...
  /**
   * Returns {@code this} if this {@code Try} is a {@link Failure} or if it is a {@link Success} and
   * the value satisfies the given checked predicate.
   *
   * <p>If this is a {@link Success} and the predicate returns {@code false}, or if evaluating the
   * predicate throws an exception, a new {@link Failure} is returned. The returned failure wraps a
   * {@link Throwable} provided by the given {@code throwableSupplier}.
   *
   * @param predicate the checked predicate to test the value
   * @param throwableSupplier a supplier providing the {@link Throwable} for the failure if the
   *     predicate does not hold
   * @return this {@code Try} if it is a {@link Failure} or the predicate passes, otherwise a {@link
   *     Failure} from the supplier
   * @throws NullPointerException if {@code predicate} or {@code throwableSupplier} is {@code null}
   */
  default Try<T> filterTry(
      @NonNull CheckedPredicate<? super T> predicate,
   ...
   * the value satisfies the given checked predicate.
   *
   * <p>If the predicate does not hold or throws an exception, a new {@link Failure} is returned.
   * The returned failure wraps a {@link Throwable} provided by the given {@code errorProvider}
   * function.
   *
   * @param predicate the checked predicate to test the value
   * @param errorProvider a function that provides a {@link Throwable} if the predicate fails
   * @return this {@code Try} if it is a {@link Failure} or the predicate passes, otherwise a {@link
   *     Failure} from the error provider
   * @throws NullPointerException if {@code predicate} or {@code errorProvider} is {@code null}
   */
  default Try<T> filterTry(
      @NonNull CheckedPredicate<? super T> predicate,
   ...
   * the value satisfies the given checked predicate.
   *
   * <p>If the predicate does not hold or throws an exception, a new {@link Failure} wrapping a
   * {@link NoSuchElementException} is returned.
   *
   * @param predicate the checked predicate to test the value
   * @return this {@code Try} if it is a {@link Failure} or the predicate passes, otherwise a {@link
   *     Failure} with a {@link NoSuchElementException}
   * @throws NullPointerException if {@code predicate} is {@code null}
   */
  default Try<T> filterTry(@NonNull CheckedPredicate<? super T> predicate) {
    Objects.requireNonNull(predicate, "predicate is null");
    return filterTry(
        predicate, () -> new NoSuchElementException("Predicate does not hold for " + get()));
  }

  /**
   * Transforms the value of this {@code Try} using the given {@link Function} if it is a {@link
   * Success}, or returns this {@link Failure}.
   *
   * <p>This is a shortcut for {@link #flatMapTry(CheckedFunction1)}.
   *
   * @param mapper a function mapping the value to another {@code Try}
   * @param <U> the type of the resulting {@code Try}
   * @return a new {@code Try} resulting from applying the mapper, or this {@code Failure} if this
   *     is a failure
  ...
  /**
   * Transforms the value of this {@code Try} using the given {@link CheckedFunction1} if it is a
   * {@link Success}, or returns this {@link Failure}.
   *
   * <p>If applying the mapper throws an exception, a {@link Failure} containing the exception is
   * returned.
   *
  ...
  default <U> Try<U> flatMapTry(
      @NonNull CheckedFunction1<? super T, ? extends Try<? extends U>> mapper) {
    Objects.requireNonNull(mapper, "mapper is null");
    if (isFailure()) {
  ...
  /**
   * Returns the value of this {@code Try} if it is a {@link Success}, or throws the underlying
   * exception if it is a {@link Failure}.
   *
   * <p><strong>Important:</strong> If this {@code Try} is a {@link Failure}, the exception thrown
   * is exactly the {@link #getCause()} of this {@code Failure}.
   *
   * @return the value contained in this {@code Success} throws Throwable the underlying cause
   *     sneakily if this is a {@link Failure}
   */
  @Override
  T get();
   ...
   *
   * <p>If this {@code Try} is a {@link Success}, it is returned unchanged. If this is a {@link
   * Failure}, the cause is matched against the provided {@code cases}. If a match is found, a new
   * {@link Failure} containing the mapped exception is returned. If none of the cases match, the
   * original {@link Failure} is returned.
   *
  ...
  default Try<T> mapFailure(
      Match.@NonNull Case<? extends Throwable, ? extends Throwable> @NonNull ... cases) {
    if (isSuccess()) {
      return this;
    } else {
  ...
  /**
   * Applies the given checked function to the value of this {@link Success}, or returns this {@link
   * Failure} unchanged.
   *
   * <p>If the function throws an exception, a new {@link Failure} containing the exception is
   * returned. This allows chaining of computations that may throw checked exceptions.
   *
   * <p>Example:
   *
   * <pre>{@code
   ...
   * @return a new {@code Try} containing the mapped value if this is a {@link Success}, otherwise
   *     this {@link Failure}
   * @throws NullPointerException if {@code mapper} is {@code null}
   */
  @SuppressWarnings("unchecked")
  ...
  /**
   * Performs the given action if this {@link Try} is a {@link Failure} and the cause is an instance
   ...
   */
  @SuppressWarnings("unchecked")
  default <X extends Throwable> Try<T> onFailure(
      @NonNull Class<X> exceptionType, @NonNull Consumer<? super X> action) {
  ...
  /**
   * Returns this {@code Try} if it is a {@link Success}, or the given alternative {@code Try} if
  ...
  /**
   * Returns this {@code Try} if it is a {@link Success}, or a {@code Try} supplied by the given
   * {@link Supplier} if this is a {@link Failure}.
   *
   * <p>The supplier is only invoked if this {@code Try} is a {@link Failure}.
   *
  ...
  /**
   * Returns the value of this {@code Success}, or applies the given function to the cause if this
   * is a {@link Failure}.
   *
   * @param other a function mapping the throwable cause to a replacement value
   * @return the value of this {@link Success}, or the result of applying {@code other} to the
   *     failure cause
   * @throws NullPointerException if {@code other} is null
   */
   ...
   *
   * <p>This method is typically used for side-effecting handling of failure cases.
   *
   * @param action a consumer of the throwable cause
   * @throws NullPointerException if {@code action} is null
   */
  ...
  /**
   * Returns the value of this {@link Success}, or throws a provided exception if this is a {@link
   * Failure}.
   *
   * <p>The exception to throw is created by applying the given {@code exceptionProvider} function
   ...
   */
  default <X extends Throwable> T getOrElseThrow(
      @NonNull Function<? super Throwable, X> exceptionProvider) throws X {
  ...
  /**
   * Folds this {@code Try} into a single value by applying one of two functions: one for the
   * failure case and one for the success case.
   *
   * <p>If this is a {@link Failure}, the {@code ifFail} function is applied to the cause. If this
   * is a {@link Success}, the {@code f} function is applied to the value.
   *
   ...
   */
  default <X> X fold(
      @NonNull Function<? super Throwable, ? extends X> ifFail,
   ...
   *
   * <p>This method is useful for side-effecting operations without transforming the value.
   *
   * @param action a consumer of the value
   * @return this {@code Try} instance
   * @throws NullPointerException if {@code action} is null
   ...
   *
   * <p>If this {@code Try} is a {@link Success}, it is returned unchanged. If this is a {@link
   * Failure} and the cause is assignable from {@code exceptionType}, the recovery function {@code
   * f} is applied to the cause inside a new {@code Try}. Otherwise, the original {@code Failure} is
   * returned.
   *
   * <p>Example:
   *
   ...
   * @return a {@code Success} with the recovered value if the exception matches, otherwise this
   *     {@code Try}
   * @throws NullPointerException if {@code exceptionType} or {@code f} is null
   */
  @SuppressWarnings("unchecked")
  default <X extends Throwable> Try<T> recover(
      @NonNull Class<X> exceptionType, @NonNull Function<? super X, ? extends T> f) {
    Objects.requireNonNull(exceptionType, "exceptionType is null");
  ...
  /**
   * Attempts to recover from a failure by applying the given recovery function if the cause matches
   * the specified exception type.
   *
   * <p>If this {@code Try} is a {@link Success}, it is returned unchanged. If this is a {@link
   * Failure} and the cause is assignable from {@code exceptionType}, the recovery function {@code
   * f} is applied to the cause and returns a new {@code Try}. If the returned {@code Try} is a
   * {@link Try#isFailure()}, the recovery is considered unsuccessful. Otherwise, the original
   * {@code Failure} is returned.
   *
   * <p>Example:
   *
   * <pre>{@code
   ...
   * @param f a recovery function that takes an exception of type {@code X} and returns a new {@code
   *     Try} instance
   * @return a {@code Try} representing the recovered value if the exception matches, otherwise this
   *     {@code Try}
   * @throws NullPointerException if {@code exceptionType} or {@code f} is null
   */
  @SuppressWarnings("unchecked")
  default <X extends Throwable> Try<T> recoverWith(
      @NonNull Class<X> exceptionType, @NonNull Function<? super X, Try<? extends T>> f) {
    Objects.requireNonNull(exceptionType, "exceptionType is null");
    Objects.requireNonNull(f, "f is null");
    if (isFailure()) {
      final Throwable cause = getCause();
  ...
  /**
   * Recovers this {@code Try} with the given {@code recovered} value if this is a {@link
   * Try.Failure} and the underlying cause is assignable to the specified {@code exceptionType}.
   *
   * <p>If this {@code Try} is a {@link Success}, or if the cause does not match {@code
   * exceptionType}, the original {@code Try} is returned unchanged.
   *
   * <p>Example:
   *
   * <pre>{@code
   ...
   */
  default <X extends Throwable> Try<T> recoverWith(
      @NonNull Class<X> exceptionType, @NonNull Try<? extends T> recovered) {
    Objects.requireNonNull(exceptionType, "exceptionType is null");
  ...
  /**
   * Recovers this {@code Try} with the given {@code value} if this is a {@link Try.Failure} and the
   * underlying cause matches the specified {@code exceptionType}.
   *
   * <p>If this {@code Try} is a {@link Success}, or if the cause is not assignable from {@code
   * exceptionType}, the original {@code Try} is returned unchanged.
   *
   * <p>Example:
   *
   ...
   * @return a {@code Try} containing the recovery value if the exception matches, otherwise this
   *     {@code Try}
  ...
  /**
   * Recovers this {@code Try} if it is a {@link Try.Failure} by applying the given recovery
   * function {@code f} to the underlying exception. The result of the function is wrapped in a
   * {@link Try.Success}.
   *
   * <p>If this {@code Try} is already a {@link Success}, it is returned unchanged.
   *
   * <p>Example:
   *
   * <pre>{@code
  ...
  /**
   * Recovers this {@code Try} if it is a {@link Try.Failure} by applying the given recovery
   * function {@code f} to the underlying exception. The recovery function returns a new {@code Try}
   * instance.
   *
   * <p>If this {@code Try} is already a {@link Success}, it is returned unchanged. If an exception
   * occurs while executing the recovery function, a new {@link Try.Failure} is returned wrapping
   * that exception.
   *
   * <p>Example:
   *
  ...
  /**
   * Recovers from any failure by evaluating the given {@code recoveryAttempt} if this {@code Try}
   * is a {@link Try.Failure}.
   *
   * <p>If this {@code Try} is already a {@link Success}, it is returned unchanged. The {@code
   * recoveryAttempt} is evaluated using {@link Try#of(CheckedFunction0)}, and its result is wrapped
   * in a new {@code Try}.
   *
   * <p>Example:
   *
   ...
   * @return a {@code Try} containing either the original success value or the result of {@code
   *     recoveryAttempt}
  ...
  /**
   * Returns {@code this} if it is a {@link Try.Success}, or attempts to recover from a failure when
   * the underlying cause is assignable to the specified {@code exceptionType} by evaluating the
   * given {@code recoveryAttempt} (via {@link Try#of(CheckedFunction0)}).
   *
   * <p>Example usage:
   *
   * <pre>{@code
  ...
  default <X extends Throwable> Try<T> recoverAndTry(
      @NonNull Class<X> exceptionType, @NonNull CheckedFunction0<? extends T> recoveryAttempt) {
    Objects.requireNonNull(exceptionType, "exceptionType is null");
    Objects.requireNonNull(recoveryAttempt, "recoveryAttempt is null");
    return isFailure() && exceptionType.isAssignableFrom(getCause().getClass())
        ? of(recoveryAttempt)
   ...
   *
   * <p>If this is a {@link Try.Success}, the value is wrapped as a {@link Either#right(Object)}. If
   * this is a {@link Try.Failure}, the cause is wrapped as a {@link Either#left(Object)}.
   *
   ...
   */
  default <L> Either<L, T> toEither(
      @NonNull Function<? super Throwable, ? extends L> throwableMapper) {
   ...
   *
   * <p>A {@link Try.Success} is converted to a {@link Validation#valid(Object)}, while a {@link
   * Try.Failure} is converted to a {@link Validation#invalid(Object)} containing the cause.
   *
  ...
  /**
   * Converts this {@code Try} to a {@link Validation}, mapping the failure cause to an invalid
   * value using the provided {@link Function}.
   *
   * <pre>{@code
   ...
   * @throws NullPointerException if {@code throwableMapper} is null
   */
  default <U> Validation<U, T> toValidation(
      @NonNull Function<? super Throwable, ? extends U> throwableMapper) {
    Objects.requireNonNull(throwableMapper, "throwableMapper is null");
    if (isFailure()) {
  ...
  /**
   * Executes a given {@link Runnable} after this {@code Try}, regardless of whether it is a {@link
   * Try.Success} or {@link Try.Failure}.
   *
   * @param runnable a final action to perform
   * @return this {@code Try} unchanged if the runnable succeeds, or a new {@link Try.Failure} if it
   *     throws
   ...
   *
   * <p>Instances of this class indicate that the computation completed successfully and hold the
   * resulting value of type {@code T}.
   *
   * <pre>{@code
   ...
   *
   * <p>Instances of this class indicate that the computation threw an exception and do not contain
   * a successful value.
   *
   * <pre>{@code
  ...
  final class Failure<T> implements Try<T>, Serializable {
     ...
     * @throws NullPointerException if {@code cause} is null throws Throwable sneakily, if the given
     *     {@code cause} is fatal, i.e. non-recoverable
     */
    ...
    public boolean equals(Object obj) {
      return (obj == this)
          || (obj instanceof Failure
              && Arrays.deepEquals(
                  cause.getStackTrace(), ((Failure<?>) obj).cause.getStackTrace()));
    }
  ...
  /**
   * Creates a {@code Try}-with-resources builder that operates on one {@link AutoCloseable}
   * resource.
   *
   ...
   */
  static <T1 extends AutoCloseable> WithResources1<T1> withResources(
      @NonNull CheckedFunction0<? extends T1> t1Supplier) {
    return new WithResources1<>(t1Supplier);
  }

  /**
   * Creates a {@code Try}-with-resources builder that operates on two {@link AutoCloseable}
   * resources.
   *
   ...
   */
  static <T1 extends AutoCloseable, T2 extends AutoCloseable> WithResources2<T1, T2> withResources(
      @NonNull CheckedFunction0<? extends T1> t1Supplier,
  ...
  /**
   * Creates a {@code Try}-with-resources builder that operates on three {@link AutoCloseable}
   * resources.
   *
   * @param t1Supplier The supplier of the 1st resource.
   * @param t2Supplier The supplier of the 2nd resource.
  ...
  static <T1 extends AutoCloseable, T2 extends AutoCloseable, T3 extends AutoCloseable>
      WithResources3<T1, T2, T3> withResources(
          @NonNull CheckedFunction0<? extends T1> t1Supplier,
          @NonNull CheckedFunction0<? extends T2> t2Supplier,
  ...
  /**
   * Creates a {@code Try}-with-resources builder that operates on four {@link AutoCloseable}
   * resources.
   *
   * @param t1Supplier The supplier of the 1st resource.
   * @param t2Supplier The supplier of the 2nd resource.
  ...
  static <
      ...
      WithResources4<T1, T2, T3, T4> withResources(
          @NonNull CheckedFunction0<? extends T1> t1Supplier,
          @NonNull CheckedFunction0<? extends T2> t2Supplier,
          @NonNull CheckedFunction0<? extends T3> t3Supplier,
  ...
  /**
   * Creates a {@code Try}-with-resources builder that operates on five {@link AutoCloseable}
   * resources.
   *
   * @param t1Supplier The supplier of the 1st resource.
   * @param t2Supplier The supplier of the 2nd resource.
  ...
  static <
      ...
      WithResources5<T1, T2, T3, T4, T5> withResources(
          @NonNull CheckedFunction0<? extends T1> t1Supplier,
          @NonNull CheckedFunction0<? extends T2> t2Supplier,
          @NonNull CheckedFunction0<? extends T3> t3Supplier,
          @NonNull CheckedFunction0<? extends T4> t4Supplier,
  ...
  /**
   * Creates a {@code Try}-with-resources builder that operates on six {@link AutoCloseable}
   * resources.
   *
   * @param t1Supplier The supplier of the 1st resource.
   * @param t2Supplier The supplier of the 2nd resource.
  ...
  static <
      ...
      WithResources6<T1, T2, T3, T4, T5, T6> withResources(
          @NonNull CheckedFunction0<? extends T1> t1Supplier,
          @NonNull CheckedFunction0<? extends T2> t2Supplier,
          @NonNull CheckedFunction0<? extends T3> t3Supplier,
          @NonNull CheckedFunction0<? extends T4> t4Supplier,
    ...
    return new WithResources6<>(
        t1Supplier, t2Supplier, t3Supplier, t4Supplier, t5Supplier, t6Supplier);
  }

  /**
   * Creates a {@code Try}-with-resources builder that operates on seven {@link AutoCloseable}
   * resources.
   *
   * @param t1Supplier The supplier of the 1st resource.
   * @param t2Supplier The supplier of the 2nd resource.
  ...
  static <
      ...
      WithResources7<T1, T2, T3, T4, T5, T6, T7> withResources(
          @NonNull CheckedFunction0<? extends T1> t1Supplier,
          @NonNull CheckedFunction0<? extends T2> t2Supplier,
          @NonNull CheckedFunction0<? extends T3> t3Supplier,
          @NonNull CheckedFunction0<? extends T4> t4Supplier,
    ...
    return new WithResources7<>(
        t1Supplier, t2Supplier, t3Supplier, t4Supplier, t5Supplier, t6Supplier, t7Supplier);
  }

  /**
   * Creates a {@code Try}-with-resources builder that operates on eight {@link AutoCloseable}
   * resources.
   *
   * @param t1Supplier The supplier of the 1st resource.
   * @param t2Supplier The supplier of the 2nd resource.
  ...
  static <
      ...
      WithResources8<T1, T2, T3, T4, T5, T6, T7, T8> withResources(
          @NonNull CheckedFunction0<? extends T1> t1Supplier,
          @NonNull CheckedFunction0<? extends T2> t2Supplier,
          @NonNull CheckedFunction0<? extends T3> t3Supplier,
          @NonNull CheckedFunction0<? extends T4> t4Supplier,
   ...
   * @param <T3> Type of the 3rd resource.
   */
  final class WithResources3<
      T1 extends AutoCloseable, T2 extends AutoCloseable, T3 extends AutoCloseable> {

    private final CheckedFunction0<? extends T1> t1Supplier;
    private final CheckedFunction0<? extends T2> t2Supplier;
    private final CheckedFunction0<? extends T3> t3Supplier;

    private WithResources3(
        @NonNull CheckedFunction0<? extends T1> t1Supplier,
        @NonNull CheckedFunction0<? extends T2> t2Supplier,
        @NonNull CheckedFunction0<? extends T3> t3Supplier) {
      this.t1Supplier = t1Supplier;
    ...
    @SuppressWarnings("try") /* https://bugs.openjdk.java.net/browse/JDK-8155591 */
    public <R> Try<R> of(
        @NonNull CheckedFunction3<? super T1, ? super T2, ? super T3, ? extends R> f) {
      return Try.of(
          () -> {
            try (T1 t1 = t1Supplier.apply();
                T2 t2 = t2Supplier.apply();
                T3 t3 = t3Supplier.apply()) {
  ...
  final class WithResources4<
    ...
    @SuppressWarnings("try") /* https://bugs.openjdk.java.net/browse/JDK-8155591 */
    public <R> Try<R> of(
        @NonNull CheckedFunction4<? super T1, ? super T2, ? super T3, ? super T4, ? extends R> f) {
      return Try.of(
          () -> {
            try (T1 t1 = t1Supplier.apply();
                T2 t2 = t2Supplier.apply();
                T3 t3 = t3Supplier.apply();
                T4 t4 = t4Supplier.apply()) {
  ...
  final class WithResources5<
    ...
    public <R> Try<R> of(
        @NonNull
            CheckedFunction5<
                ? super T1, ? super T2, ? super T3, ? super T4, ? super T5, ? extends R>
            f) {
      return Try.of(
          () -> {
            try (T1 t1 = t1Supplier.apply();
  ...
  final class WithResources6<
    ...
    public <R> Try<R> of(
        @NonNull
            CheckedFunction6<
                ? super T1, ? super T2, ? super T3, ? super T4, ? super T5, ? super T6, ? extends R>
            f) {
      return Try.of(
          () -> {
            try (T1 t1 = t1Supplier.apply();
