 ...
 *
 * <p>{@code Option} is a monadic container type representing the presence or absence of a value. An
 * instance is either a {@link Some} holding a value or the singleton {@link None}.
 *
 * <p>The design is similar to {@link java.util.Optional} and related types in <a
...
public interface Option<T> extends Value<T>, Serializable {
  ...
  /**
   * Reduces multiple {@code Option} values into a single {@code Option} by transforming an {@code
   * Iterable<Option<? extends T>>} into an {@code Option<Seq<T>>}.
   *
   * <p>If any element is {@link Option.None}, the result is {@code None}. Otherwise, all contained
   * values are collected into a {@link Seq} wrapped in {@code Some}.
   *
   * @param values an iterable of {@code Option} values
   * @param <T> the element type
   * @return an {@code Option} containing a {@code Seq} of all values, or {@code None} if any value
   *     is empty
  ...
  /**
   * Maps the elements of an iterable into {@code Option} values and collects the results into a
   * single {@code Option}.
   *
   * <p>Each element is transformed using {@code mapper}. If any mapped value is {@link
   * Option.None}, the result is {@code None}. Otherwise, all mapped values are accumulated into a
   * {@link Seq} wrapped in {@code Some}.
   *
   ...
   * @return an {@code Option} containing a {@code Seq} of mapped values, or {@code None} if any
   *     mapping yields {@code None}
   ...
   *
   * <p>Unlike {@link Option#of(Object)}, this method preserves {@code null}:
   *
   * <pre>
   ...
   *
   * <p>This is safe because immutable/read-only types are covariant.
   *
  ...
  /**
   * Returns {@code Some} of the value supplied by {@code supplier} if {@code condition} is true, or
   * {@code None} if {@code condition} is false.
   *
   * @param <T> the type of the optional value
  ...
  /**
   * Returns {@code Some} of the given {@code value} if {@code condition} is true, or {@code None}
   * otherwise.
   *
   * @param <T> the type of the optional value
   * @param condition the condition to test
  ...
  /**
   * Applies a {@code partialFunction} to the value of this {@code Option} if it is defined for that
   * value, and wraps the result in a new {@code Option}.
   *
   * <p>If the {@code partialFunction} is not defined for the value, {@code None} is returned.
   *
   * <pre>{@code
   ...
   *
   * <p>Note that {@code Some(null)} is considered defined.
   *
  ...
  /**
   * Returns the value contained in this {@code Some}, or the provided {@code other} value if this
   * is {@code None}.
   *
   * <p>Note that {@code other} is evaluated eagerly.
   *
  ...
  /**
   * Returns this {@code Option} if it is non-empty, otherwise returns the provided alternative
   * {@code Option}.
   *
  ...
  /**
   * Returns this {@code Option} if it is non-empty; otherwise, returns the {@code Option} provided
   * by the supplier.
   *
  ...
  /**
   * Returns the value contained in this {@code Some}, or the value supplied by {@code supplier} if
   * this is {@code None}.
   *
   * <p>The alternative value is evaluated lazily.
   *
  ...
  /**
   * Returns the value contained in this {@code Some}, or throws an exception provided by {@code
   * exceptionSupplier} if this is {@code None}.
   *
  ...
  /**
   * Returns {@code Some(value)} if this {@code Option} is a {@code Some} and the contained value
   * satisfies the given predicate. Otherwise, returns {@code None}.
   *
  ...
  /**
   * Transforms the value of this {@code Some} using the given mapper and wraps it in a new {@code
   * Some}. Returns {@code None} if this is {@code None}.
   *
   * @param mapper a function to transform the contained value
  ...
  /**
   * Converts this {@code Option} to a {@link Try}, then applies the given checked function if this
   * is a {@link Try.Success}, passing the contained value to it.
   *
   * @param <U> the type of the resulting {@code Try}'s value
   * @param mapper a checked function to transform the contained value
   * @return a {@code Try} containing the mapped value if this {@code Option} is defined, otherwise
   *     a {@link Try.Failure}
   * @throws NullPointerException if {@code mapper} is null
   */
   ...
   *
   * <ul>
   *   <li>{@code ifNone} is applied if this is {@code None}
   *   <li>{@code f} is applied to the contained value if this is {@code Some}
   * </ul>
   *
   ...
   * @return the result of applying {@code f} or {@code ifNone} depending on whether this is {@code
   *     Some} or {@code None}
   * @throws NullPointerException if {@code ifNone} or {@code f} is null
   */
  default <U> U fold(
      @NonNull Supplier<? extends U> ifNone, @NonNull Function<? super T, ? extends U> f) {
    return this.<U>map(f).getOrElse(ifNone);
  }

  /**
   * Executes the given action on the contained value if this {@code Option} is defined ({@code
   * Some}), otherwise does nothing.
   *
