 ...
 *
 * <p>An {@code Either<L, R>} is typically used to model a computation that may result in either a
 * success (represented by {@code Right}) or a failure (represented by {@code Left}).
 *
 * <p>This implementation is <strong>right-biased</strong>, meaning that most operations such as
 ...
 * <h2>Example</h2>
 *
 * <p>Suppose we have a {@code compute()} function that returns an {@code Either<String, Integer>},
 ...
 * }</pre>
 *
 * <p>If {@code compute()} returns {@code Right(1)}, the result will be {@code Right(2)}.<br>
 * If {@code compute()} returns {@code Left("error")}, the result will remain {@code Left("error")}.
 *
 * <h2>Projection Semantics</h2>
 *
 * <ul>
 *   <li>If an {@code Either} is a {@code Right} and projected to {@code Left}, operations on {@code
 *       Left} are no-ops.
 *   <li>If an {@code Either} is a {@code Left} and projected to {@code Right}, operations on {@code
 *       Right} are no-ops.
 *   <li>Operations on the matching projection are applied as expected.
 * </ul>
 *
...
public interface Either<L, R> extends Value<R>, Serializable {
  ...
  /**
   * Narrows a {@code Either<? extends L, ? extends R>} to {@code Either<L, R>} via a type-safe
   * cast. This is safe because immutable or read-only collections are covariant.
   *
   ...
   *
   * <ul>
   *   <li>If {@code test} is {@code true}, the result is a {@link Either.Right} created from {@code
   *       right}.
   *   <li>If {@code test} is {@code false}, the result is a {@link Either.Left} created from {@code
   *       left}.
   * </ul>
   *
  ...
  static <L, R> Either<L, R> cond(
      boolean test, @NonNull Supplier<? extends R> right, @NonNull Supplier<? extends L> left) {
   ...
   *
   * <ul>
   *   <li>If {@code test} is {@code true}, the result is a {@link Either.Right} containing {@code
   *       right}.
   *   <li>If {@code test} is {@code false}, the result is a {@link Either.Left} containing {@code
   *       left}.
   * </ul>
   *
   * @param test the boolean condition to evaluate
   ...
   * @param <L> the type of the left value
   * @param <R> the type of the right value
   * @return an {@code Either<L, R>} containing either the left or right value depending on {@code
   *     test}
   * @throws NullPointerException if any argument is null
   */
   ...
   *
   * <ul>
   *   <li>If this is a {@link Either.Left}, {@code leftMapper} is applied to the left value.
   *   <li>If this is a {@link Either.Right}, {@code rightMapper} is applied to the right value.
   * </ul>
   *
   ...
   */
  default <X, Y> Either<X, Y> bimap(
      @NonNull Function<? super L, ? extends X> leftMapper,
  ...
  /**
   * Transforms an {@link Iterable} of {@code Either<L, R>} into a single {@code Either<Seq<L>,
   * Seq<R>>}.
   *
   * <p>If any of the given {@code Either}s is a {@link Either.Left}, the result is a {@link
   * Either.Left} containing a non-empty {@link Seq} of all left values.
   *
   * <p>If all of the given {@code Either}s are {@link Either.Right}, the result is a {@link
   * Either.Right} containing a (possibly empty) {@link Seq} of all right values.
   *
   * <pre>{@code
   * // = Right(Seq())
   ...
   * @return an {@code Either} containing a {@link Seq} of left values if any {@code Either} was a
   *     {@link Either.Left}, otherwise a {@link Seq} of right values
   * @throws NullPointerException if {@code eithers} is null
   */
  @SuppressWarnings("unchecked")
  static <L, R> Either<Seq<L>, Seq<R>> sequence(
      @NonNull Iterable<? extends Either<? extends L, ? extends R>> eithers) {
    ...
    return Iterator.ofAll((Iterable<Either<L, R>>) eithers)
        ...
        .apply(
            (leftPartition, rightPartition) ->
                leftPartition.hasNext()
                    ? Either.left(leftPartition.map(Either::getLeft).toVector())
                    : Either.right(rightPartition.map(Either::get).toVector()));
  }

  /**
   * Transforms an {@link Iterable} of values into a single {@code Either<Seq<L>, Seq<R>>} by
   * applying a mapping function that returns an {@code Either} for each value.
   *
   * <p>If the mapper returns any {@link Either.Left}, the resulting {@code Either} is a {@link
   * Either.Left} containing a {@link Seq} of all left values. Otherwise, the result is a {@link
   * Either.Right} containing a {@link Seq} of all right values.
   *
   ...
   *
   * <p>If any of the given {@code Either}s is a {@link Either.Left}, the result is a {@link
   * Either.Left} containing the first left value encountered in iteration order.
   *
   * <p>If all of the given {@code Either}s are {@link Either.Right}, the result is a {@link
   * Either.Right} containing a (possibly empty) {@link Seq} of all right values.
   *
   ...
   * @param <L> the type of left values
   * @param <R> the type of right values
   * @return an {@code Either} containing either the first left value if present, or a {@link Seq}
   *     of all right values
   * @throws NullPointerException if {@code eithers} is null
   */
  ...
  /**
   * Transforms an {@link Iterable} of values into a single {@code Either<Seq<L>, Seq<R>>} by
   * applying a mapping function that returns an {@code Either} for each element.
   *
   * <p>If the mapper returns any {@link Either.Left}, the resulting {@code Either} is a {@link
   * Either.Left} containing a {@link Seq} of all left values. Otherwise, the result is a {@link
   * Either.Right} containing a {@link Seq} of all right values.
  ...
  /**
   * Returns the right value of this {@code Either}, or an alternative value if this is a {@link
   * Either.Left}.
   *
   * @param other a function that converts a left value to an alternative right value
   * @return the right value if present, otherwise the alternative value produced by applying {@code
   *     other} to the left value
   */
  ...
  /**
   * Returns the right value of this {@code Either}, or throws an exception if it is a {@link
   * Either.Left}.
   *
   ...
   * @throws X if this {@code Either} is a {@link Either.Left}, using the exception produced by
   *     {@code exceptionFunction}
   */
  ...
  /**
   * Swaps the sides of this {@code Either}, converting a {@link Either.Left} to a {@link
   * Either.Right} and vice versa.
   *
   ...
   *
   * <p>If this {@code Either} is a {@link Either.Left}, it is returned unchanged. Otherwise, the
   * {@code mapper} function is applied to the right value, and its result is returned.
   *
   ...
   * @return this {@code Either} unchanged if it is a {@link Either.Left}, or the result of applying
   *     {@code mapper} if it is a {@link Either.Right}
   * @throws NullPointerException if {@code mapper} is null
   */
  @SuppressWarnings("unchecked")
  default <U> Either<L, U> flatMap(
      @NonNull Function<? super R, ? extends Either<L, ? extends U>> mapper) {
    Objects.requireNonNull(mapper, "mapper is null");
    if (isRight()) {
   ...
   *
   * <p>If this {@code Either} is a {@link Either.Left}, no operation is performed and it is
   * returned unchanged.
   *
   * <pre>{@code
   ...
   *
   * <p>If this {@code Either} is a {@link Either.Right}, no operation is performed and it is
   * returned unchanged.
   *
   * <pre>{@code
   * import static io.vavr.API.*;
  ...
  /**
   * Returns an {@link Option} describing the right value of this right-biased {@code Either} if it
   * satisfies the given predicate.
   *
   * <p>If this {@code Either} is a {@link Either.Left} or the predicate does not match, {@link
   * Option#none()} is returned.
   *
   * @param predicate a predicate to test the right value
   * @return an {@link Option} containing the right value if it satisfies the predicate, or {@link
   ...
   *
   * <p>If this {@code Either} is a {@link Either.Right} and the predicate evaluates to {@code
   * false}, the result is a {@link Either.Left} obtained by applying the {@code zero} function to
   * the right value. If the predicate evaluates to {@code true}, the {@code Either.Right} is
   * returned unchanged.
   *
   * <pre>{@code
   * import static io.vavr.API.*;
   ...
   * @return an {@code Either} containing the right value if the predicate matches, or a left value
   *     otherwise
   * @throws NullPointerException if {@code predicate} or {@code zero} is null
   */
  default Either<L, R> filterOrElse(
      @NonNull Predicate<? super R> predicate, @NonNull Function<? super R, ? extends L> zero) {
    Objects.requireNonNull(predicate, "predicate is null");
  ...
  /**
   * Returns this {@code Either} if it is a {@link Either.Right}, otherwise returns the given {@code
   * other} Either.
   *
  ...
  /**
   * Returns this {@code Either} if it is a {@link Either.Right}, otherwise returns the result of
   * evaluating the given {@code supplier}.
   *
   ...
   */
  @SuppressWarnings("unchecked")
  default Either<L, R> orElse(
      @NonNull Supplier<? extends Either<? extends L, ? extends R>> supplier) {
   ...
   *
   * <p>If this is a {@link Either.Right}, no action is performed.
   *
   * @param action a consumer that processes the left value
   ...
   *
   * @return {@code Validation.valid(get())} if this is right, otherwise {@code
   *     Validation.invalid(getLeft())}.
   */
  ...
  @Override
  default Try<R> toTry() {
    return isRight() ? Try.success(get()) : Try.failure(new Failure(getLeft()));
  }
  ...
  final class LeftProjection<L, R> implements Value<L> {
    ...
    /**
     * Applies transformations to the underlying {@code Either} by delegating to {@link
     * Either#bimap(Function, Function)} and returns the left projection of the result.
     *
     * <ul>
     *   <li>If the underlying {@code Either} is a {@link Either.Left}, {@code leftMapper} is
     *       applied to the left value.
     *   <li>If the underlying {@code Either} is a {@link Either.Right}, {@code rightMapper} is
     *       applied to the right value.
     * </ul>
     *
     ...
     */
    public <L2, R2> LeftProjection<L2, R2> bimap(
        Function<? super L, ? extends L2> leftMapper,
        @NonNull Function<? super R, ? extends R2> rightMapper) {
     ...
     * @throws NoSuchElementException if the underlying {@code Either} of this {@code
     *     LeftProjection} is a {@code Right}
     */
    @Override
    public L get() {
      if (either.isLeft()) {
    ...
    /**
     * Returns this {@code LeftProjection} if the underlying {@code Either} is a {@link
     * Either.Left}, otherwise returns the given {@code other} projection.
     *
     * @param other an alternative {@code LeftProjection}
     * @return this projection if the underlying {@code Either} is a {@code Left}, otherwise {@code
     *     other}
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Returns this {@code LeftProjection} if the underlying {@code Either} is a {@link
     * Either.Left}, otherwise returns the result of evaluating the given {@code supplier}.
     *
     * @param supplier a supplier of an alternative {@code LeftProjection}
     * @return this projection if the underlying {@code Either} is a {@code Left}, otherwise the
     *     result of {@code supplier}
     */
    @SuppressWarnings("unchecked")
    public LeftProjection<L, R> orElse(
        @NonNull Supplier<? extends LeftProjection<? extends L, ? extends R>> supplier) {
      Objects.requireNonNull(supplier, "supplier is null");
     ...
     * @return the left value, if the underlying Either is a Left or else the alternative Left value
     *     provided by {@code other} by applying the Right value.
     */
     ...
     * @return the left value, if the underlying Either is a Left or else throws the exception
     *     provided by {@code exceptionFunction} by applying the Right value.
     * @throws X if the projected Either is a Right
     */
    public <X extends Throwable> L getOrElseThrow(@NonNull Function<? super R, X> exceptionFunction)
        throws X {
      Objects.requireNonNull(exceptionFunction, "exceptionFunction is null");
      if (either.isLeft()) {
        return either.getLeft();
    ...
    /**
     * Returns {@code Some} value of type L if this is a left projection of a Left value and the
     * predicate applies to the underlying value.
     *
    ...
    public Option<LeftProjection<L, R>> filter(@NonNull Predicate<? super L> predicate) {
      Objects.requireNonNull(predicate, "predicate is null");
      return either.isRight() || predicate.test(either.getLeft())
          ? Option.some(this)
          : Option.none();
    }
     ...
     * @return this as {@code LeftProjection<L, U>} if a Right is underlying, otherwise a the
     *     mapping result of the left value.
     * @throws NullPointerException if {@code mapper} is null
     */
    @SuppressWarnings("unchecked")
    public <U> LeftProjection<U, R> flatMap(
        @NonNull Function<? super L, ? extends LeftProjection<? extends U, R>> mapper) {
      Objects.requireNonNull(mapper, "mapper is null");
      if (either.isLeft()) {
    ...
    /**
     * Applies the given action to the value if the projected either is a Left. Otherwise nothing
     * happens.
     *
    ...
    public boolean equals(Object obj) {
      return (obj == this)
          || (obj instanceof LeftProjection
              && Objects.equals(either, ((LeftProjection<?, ?>) obj).either));
    }
  ...
  final class RightProjection<L, R> implements Value<R> {
    ...
    /**
     * Applies transformations to the underlying {@code Either} by delegating to {@link
     * Either#bimap(Function, Function)} and returns the right projection of the result.
     *
     * <ul>
     *   <li>If the underlying {@code Either} is a {@link Either.Left}, {@code leftMapper} is
     *       applied to the left value.
     ...
     */
    public <L2, R2> RightProjection<L2, R2> bimap(
        @NonNull Function<? super L, ? extends L2> leftMapper,
        @NonNull Function<? super R, ? extends R2> rightMapper) {
     ...
     * @throws NoSuchElementException if the underlying {@code Either} of this {@code
     *     RightProjection} is a {@code Left}
     */
    @Override
    public R get() {
      if (either.isRight()) {
    ...
    /**
     * Returns this {@code RightProjection} if the underlying {@code Either} is a {@link
     * Either.Right}, otherwise returns the given {@code other} projection.
     *
     * @param other an alternative {@code RightProjection}
     * @return this projection if the underlying {@code Either} is a {@code Right}, otherwise {@code
     *     other}
     */
    @SuppressWarnings("unchecked")
    ...
    /**
     * Returns this {@code RightProjection} if the underlying {@code Either} is a {@link
     * Either.Right}, otherwise returns the result of evaluating the given {@code supplier}.
     *
     * @param supplier a supplier of an alternative {@code RightProjection}
     * @return this projection if the underlying {@code Either} is a {@code Right}, otherwise the
     *     result of {@code supplier}
     */
    @SuppressWarnings("unchecked")
    public RightProjection<L, R> orElse(
        @NonNull Supplier<? extends RightProjection<? extends L, ? extends R>> supplier) {
      Objects.requireNonNull(supplier, "supplier is null");
     ...
     *     value provided by {@code other} by applying the Left value.
     */
     ...
     *     provided by {@code exceptionFunction} by applying the Left value.
     * @throws X if the projected Either is a Left
     */
    public <X extends Throwable> R getOrElseThrow(@NonNull Function<? super L, X> exceptionFunction)
        throws X {
      Objects.requireNonNull(exceptionFunction, "exceptionFunction is null");
    ...
    /**
     * Returns {@code Some} value of type R if this is a right projection of a Right value and the
     * predicate applies to the underlying value.
     *
     * @param predicate A predicate
     * @return A new Option
     ...
     * @return this as {@code RightProjection<L, U>} if a Left is underlying, otherwise a the
     *     mapping result of the right value.
     * @throws NullPointerException if {@code mapper} is null
     */
    @SuppressWarnings("unchecked")
    public <U> RightProjection<L, U> flatMap(
        @NonNull Function<? super R, ? extends RightProjection<L, ? extends U>> mapper) {
      Objects.requireNonNull(mapper, "mapper is null");
      if (either.isRight()) {
    ...
    /**
     * Applies the given action to the value if the projected either is a Right. Otherwise nothing
     * happens.
     *
    ...
    public boolean equals(Object obj) {
      return (obj == this)
          || (obj instanceof RightProjection
              && Objects.equals(either, ((RightProjection<?, ?>) obj).either));
    }
   ...
   *
   * <p>This class wraps a value in an exception to enable using exception-based control flow when
   * working with Either. It is not possible to use a generic type parameter for the exception type
   * due to Java type system limitations.
   */
