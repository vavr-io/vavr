...
public class Node<T> extends AbstractTreeLikeStructure<T, Node<T>> implements Serializable /* TODO:extends Iterable<T> */{
	...
	public Tree<T> asTree() {
		final List<Tree<T>> treeChildren = children.stream().map(this::asTree).collect(List.collector());
	...
	private Tree<T> asTree(Node<T> node) {
		final List<Tree<T>> treeChildren = node.children.stream().map(this::asTree).collect(List.collector());
