...
interface VavrSetMixin<T, SELF extends VavrSetMixin<T, SELF>> extends io.vavr.collection.Set<T> {
    ...
    @Override
    default io.vavr.collection.Iterator<? extends Set<T>> slideBy(Function<? super T, ?> classifier) {
        return iterator().slideBy(classifier).map(this::createFromElements);
    }

    @Override
    default io.vavr.collection.Iterator<? extends Set<T>> sliding(int size) {
        return sliding(size, 1);
    }

    @Override
    default io.vavr.collection.Iterator<? extends Set<T>> sliding(int size, int step) {
