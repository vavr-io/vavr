 ...
 */
public class HashCollisionNode<D> extends Node<D> {
    ...
    Node<D> update(@Nullable IdentityObject mutator, D newData,
        ...
        for (int i = 0; i < this.data.length; i++) {
            ...
            if (equalsFunction.test(oldKey, newData)) {
                D updatedData = updateFunction.apply(oldKey, newData);
                if (updatedData == oldKey) {
                    details.found(newData);
                    return this;
                }
                details.setReplaced(oldKey, updatedData);
                if (isAllowedToUpdate(mutator)) {
